...
try {
	...
	$secondFixture = Join-Path $directory 'other-links.fb2'
...
@"
<?xml version="1.0" encoding="utf-8"?>
<FictionBook xmlns="http://www.gribuser.ru/xml/fictionbook/2.0"><description><title-info><genre>prose</genre><author><first-name>T</first-name><last-name>T</last-name></author><book-title>Other link navigation</book-title><lang>en</lang></title-info><document-info><program-used>test</program-used><id>other-link-navigation-test</id><version>1.0</version></document-info></description><body><section><p>Separate document.</p></section></body></FictionBook>
"@ | Set-Content -LiteralPath $secondFixture -Encoding utf8
    $oldMode, $oldScenario, $oldSecondFile = $env:FBE_NEXT_TEST_MODE, $env:FBE_NEXT_TEST_SCENARIO, $env:FBE_NEXT_TEST_NAVIGATION_SECOND_FILE
    try {
        $env:FBE_NEXT_TEST_MODE = '1'; $env:FBE_NEXT_TEST_SCENARIO = 'link-navigation-runtime'; $env:FBE_NEXT_TEST_NAVIGATION_SECOND_FILE = $secondFixture
    ...
    finally { $env:FBE_NEXT_TEST_MODE, $env:FBE_NEXT_TEST_SCENARIO, $env:FBE_NEXT_TEST_NAVIGATION_SECOND_FILE = $oldMode, $oldScenario, $oldSecondFile }
    $row = Import-Csv -LiteralPath $report -Delimiter "`t"
    if(@($row).Count -ne 1 -or $row.nested -ne '1' -or $row.target -ne '1' -or $row.same_document -ne '1' -or $row.broken -ne '1' -or $row.returned_second -ne '1' -or $row.inserted_before -ne '1' -or $row.inserted_returned_origin -ne '1' -or $row.deleted_origin_fallback -ne '1' -or $row.other_document_opened -ne '1' -or $row.unchanged -ne '1' -or $row.result -ne 'pass') { throw "Link navigation runtime contract failed: $($row | ConvertTo-Json -Compress)" }
