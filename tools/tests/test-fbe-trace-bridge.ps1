...
}foreach($pattern in @('var result=false;', 'var operationCompleted=false;', 'var cssRestoreFailed=false;', 'var originalException=null;', 'throw controlledFailure;', 'if(originalException)', 'throw originalException;', 'if(cssRestoreFailed)', 'TraceDiagnosticEvent("J210", "operation=CSS restore begin")', 'TraceDiagnosticEvent("J211", "operation=CSS restore success")', 'TraceDiagnosticEvent("J212", "level=error; operation=CSS restore failure; load-result=" + (operationCompleted ? "success" : "failure"))', 'diagnosticFailureStage="J212"', 'DiagError("J212", "CSS restore", e)', 'var failedStage = diagnosticFailureStage || diagnosticOperationStage || code')) {
