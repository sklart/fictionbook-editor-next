...
$modules = @($moduleFiles.Keys | Sort-Object)
    ...
    $checks = @(
        ...
        @{ Path = Join-Path (Join-Path $langRoot 'ru-RU') 'fbe.json'; Key = 'fbe.dialog.idd_setting_next.source_palette.contrast'; Expected = 'Контрастная' },
        ...
        @{ Path = Join-Path (Join-Path $langRoot 'ru-RU') 'fbe.json'; Key = 'fbe.hotkey.plugins.import'; Expected = 'Импорт' },
