...
$mainFrameImplementation = Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\mainfrm.cpp")
if ($mainFrameImplementation -notmatch '#include\s+"ui\\MainFrameRuntimeUi\.inl"') {
    throw 'mainfrm.cpp должен включать вынесенный presentation-слой runtime-локализации.'
}
$mainFrameText = $mainFrameImplementation + "`n" + (Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\ui\MainFrameRuntimeUi.inl")) + "`n" + (Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\DocumentTree.cpp")) + "`n" + (Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\DocumentTree.h")) + "`n" + (Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\ui\ContextAttributeBars.cpp")) + "`n" + (Get-Content -Raw -LiteralPath (Join-Path $repoRoot "src\fbe\source\ui\SourceEditorControl.cpp"))
foreach ($pattern in @(
