...
(function () {
  ...
  try {
    ...
    var controls={ radio1:{checked:false},radio2:{checked:false},radio3:{checked:true},radio4:{checked:false},checkbox1:{checked:false},filenameTemplate:{value:"{number}_{title}"},unicodeNames:{checked:true},outputFolder:{value:outputFolder},filenamePreview:{value:""} };
    ...
    if(calls.length!=2||calls[0]!=0||calls[1]!=1||!fso.FileExists(output)) throw new Error("myLinks batch selection did not continue after an error");
