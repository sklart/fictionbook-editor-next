...
$hotkeysStore = Text 'src\fbe\settings\hotkeys\HotkeyStore.cpp'
...
Must $frame 'CHotkey ScriptsHotkey\(identity' 'Script hotkey identity must be UID based'
Must $frame 'L"script:" \+ script\.uid' 'Script hotkey UID has explicit namespace'
MustNot $frame 'CHotkey ScriptsHotkey\(script\.path' 'Absolute script path must not be persisted as hotkey identity'
Must $hotkeysStore 'migratedLegacyScriptHotkey \|= foundHotkey != NULL' 'Legacy migrations accumulate across entries'
Must $hotkeysStore 'legacyPath\.Right\(relativePath\.GetLength\(\)\) == relativePath' 'Moved portable hotkey is matched by relative path suffix'
Must $hotkeysStore 'int longestSuffixLength = -1' 'Legacy hotkey migration tracks the most specific suffix'
Must $hotkeysStore 'suffixLength > longestSuffixLength' 'Longer nested script suffix wins over basename'
Must $hotkeysStore 'suffixLength == longestSuffixLength' 'Only equal best suffixes are ambiguous'
Must $hotkeysStore 'ambiguousLongestSuffix \? NULL : matchedHotkey' 'Ambiguous legacy hotkeys are not migrated'
...
Must $frame 'm_scripts\.ClearLastScript\(\)' 'Reload clears Last Script stable identity'
...
Must $toolbarStore 'ToolbarsV2Codec::Serialize' 'Toolbar persistence writes version 2'
Must $toolbarStore 'ToolbarsV2Codec::Parse' 'Toolbar persistence reads version 2'
Must $toolbarStore 'MOVEFILE_REPLACE_EXISTING \| MOVEFILE_WRITE_THROUGH' 'Atomic portable toolbar replacement'
...
Must $frame 'item.command = command; // deleted scripts remain unresolved' 'Deleted saved scripts are ignored'
Must $toolbarStore 'commandToolbarPresent' 'Toolbar reader distinguishes missing section from an empty section'
Must $frame 'if\(!toolbarPresent\) return;' 'Missing toolbar section keeps default layout'
Must $layoutAdapter 'while\(target\.GetButtonCount\(\) > 0\) target\.DeleteButton\(0\);' 'Explicit empty toolbar clears the default layout'
...
Must $frame 'item\.scriptUid' 'Scripts toolbar restore uses stable UID'
