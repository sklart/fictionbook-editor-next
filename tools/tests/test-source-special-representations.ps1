...
$settingsDefaults = Get-Text 'src\fbe\settings\SettingsDefaults.cpp'
$mainFrame = Get-Text 'src\fbe\mainfrm.cpp'
$sourceEditor = Get-Text 'src\fbe\source\ui\SourceEditorControl.cpp'
...
Assert-Contains $settingsDefaults 'm_xml_src_showSpecialChars\s*=\s*false' 'disabled-by-default special-character setting'
Assert-Contains $sourceEditor 'ConfigureSpecialCharacterRepresentations\s*\(' 'representation configuration hook'
Assert-Contains $sourceEditor 'SCI_SETREPRESENTATION' 'Scintilla representation setup'
Assert-Contains $sourceEditor 'SCI_CLEARREPRESENTATION' 'Scintilla representation cleanup'
Assert-Contains $sourceEditor '"NBSP"' 'NBSP representation'
Assert-Contains $sourceEditor '"\\xC2\\xB0"' 'Word-like NBSP symbol representation'
Assert-Contains $sourceEditor '"SHY"' 'soft-hyphen representation'
Assert-Contains $sourceEditor '"ZWSP"' 'zero-width-space representation'
...
Assert-Contains $mainFrame 'specialCharactersStyle' 'editor configuration snapshot special-character style'
