...
foreach ($required in @(
    'hdr->hwndFrom',
    'm_source',
    'm_editor_view_state.Current() != EditorView::Source',
...
$combinedEarlyExit = 'if\s*\(\s*hdr->hwndFrom\s*!=\s*m_source\s*\|\|\s*m_editor_view_state\.Current\(\)\s*!=\s*EditorView::Source\s*\)\s*(?:\{\s*)?return\s+0\s*;'
$sourceEarlyExit = 'if\s*\(\s*hdr->hwndFrom\s*!=\s*m_source\s*\)\s*(?:\{\s*)?return\s+0\s*;'
$viewEarlyExit = 'if\s*\(\s*m_editor_view_state\.Current\(\)\s*!=\s*EditorView::Source\s*\)\s*(?:\{\s*)?return\s+0\s*;'
