...
foreach($required in @('deployment=%s; elevated=%d; integrity=%s; trace-location=%s; settings-location=%s', 'DetectDeployment', 'IsProcessElevated', 'ProcessIntegrityLevel', 'DescribeDiagnosticModule', 'GetModuleHandle(NULL)', 'GetBinaryType', 'file-version=%s; architecture=%s', 'not-loaded', 'GetSystemDirectory', 'L"FBE.exe", true', 'L"mshtml.dll"', 'L"ieframe.dll"', 'L"urlmon.dll"', 'L"wininet.dll"', 'L"jscript.dll"', 'L"jscript9.dll"', 'L"msxml6.dll"', 'L"oleaut32.dll"', 'L"Scintilla.dll"', 'L"Lexilla.dll"')) {
    if($source.IndexOf($required, [StringComparison]::Ordinal) -lt 0) { throw "Missing environment snapshot field: $required" }
}
foreach($required in @('HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE', 'KEY_WOW64_32KEY, KEY_WOW64_64KEY', 'InstallLocation', 'installed.CompareNoCase(executableDirectory) == 0', 'return L"installed"', 'return foundOtherInstalledLocation ? L"portable" : L"unknown"')) { if($source.IndexOf($required, [StringComparison]::Ordinal) -lt 0) { throw "Missing installed deployment detection contract: $required" } }
