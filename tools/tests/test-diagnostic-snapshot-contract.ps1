...
foreach($route in @('DispatchFailure(L"script", L"C120"', 'DispatchFailure(L"script", FAILED(hr) ? L"C140"')) {
    if($fbdDoc.IndexOf($route, [StringComparison]::Ordinal) -lt 0) { throw "Missing script dispatch snapshot route: $route" }
}
foreach($route in @('DispatchFailure(L"external", L"XH120"', 'DispatchFailure(L"external", FAILED(result) ? L"XH140"')) {
