...
& cl.exe /nologo /EHsc /std:c++14 /utf-8 /DUNICODE /D_UNICODE /LD (Join-Path $PSScriptRoot 'plugin-v2-fixture.cpp') (Join-Path $root 'src\fbe\FBE_i.c') /link oleaut32.lib "/OUT:$out\plugin-v2-fixture.dll"
if ($LASTEXITCODE -ne 0) { throw 'Synthetic Plugin API v2 fixture did not compile.' }
& cl.exe /nologo /EHsc /std:c++14 /utf-8 /DUNICODE /D_UNICODE (Join-Path $PSScriptRoot 'plugin-v2-abi-harness.cpp') (Join-Path $root 'src\fbe\FBE_i.c') /link ole32.lib oleaut32.lib "/OUT:$out\plugin-v2-abi-harness.exe"
