...
& cl.exe /nologo /EHsc /std:c++14 /utf-8 /DUNICODE /D_UNICODE "/I$apiDirectory" /LD "/Fo$out\\" "/Fd$out\plugin-v2-fixture.pdb" (Join-Path $PSScriptRoot 'plugin-v2-fixture.cpp') (Join-Path $apiDirectory 'FBE_i.c') /link oleaut32.lib "/OUT:$out\plugin-v2-fixture.dll" "/IMPLIB:$out\plugin-v2-fixture.lib" "/PDB:$out\plugin-v2-fixture-link.pdb"
if ($LASTEXITCODE -ne 0) { throw 'Synthetic Plugin API v2 fixture did not compile.' }
& cl.exe /nologo /EHsc /std:c++14 /utf-8 /DUNICODE /D_UNICODE "/I$apiDirectory" "/Fo$out\\" "/Fd$out\plugin-v2-abi-harness.pdb" (Join-Path $PSScriptRoot 'plugin-v2-abi-harness.cpp') (Join-Path $apiDirectory 'FBE_i.c') /link ole32.lib oleaut32.lib "/OUT:$out\plugin-v2-abi-harness.exe" "/PDB:$out\plugin-v2-abi-harness-link.pdb"
