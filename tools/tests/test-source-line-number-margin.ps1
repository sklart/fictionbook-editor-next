...
$root=(Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path; $source=Get-Content -Raw (Join-Path $root 'src\fbe\mainfrm.cpp'); $header=Get-Content -Raw (Join-Path $root 'src\fbe\mainfrm.h')
foreach($required in @('GetLineNumberDigits','ShouldUpdateSourceLineNumberMargin','SCI_GETLINECOUNT','SCI_TEXTWIDTH','SCI_SETMARGINWIDTHN')) { if($source -notlike "*$required*"){throw "Missing source margin behavior: $required"} }
if($header -notlike '*SC_UPDATE_LINE_COUNT*') { throw 'Missing SC_UPDATE_LINE_COUNT margin update behavior.' }
$modifiedBlock = [regex]::Match($source, 'void\s+CMainFrame::SciModified\s*\([^)]*\)\s*\{[\s\S]*?(?=\n\nbool\s+CMainFrame::SciUpdateUI)')
if(!$modifiedBlock.Success) { throw 'SciModified implementation was not found.' }
if($modifiedBlock.Value -match 'SC_MOD_INSERTTEXT\s*\|\s*SC_MOD_DELETETEXT|UpdateSourceLineNumberMargin') { throw 'SciModified must not update the line-number margin for every text modification.' }
foreach($required in @('SC_MOD_CHANGEFOLD','SCI_SETFOLDEXPANDED')) { if($modifiedBlock.Value -notlike "*$required*"){throw "SciModified folding behavior is missing: $required"} }
