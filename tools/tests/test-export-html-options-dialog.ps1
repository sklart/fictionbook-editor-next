...
foreach ($token in @('BuildHtmlModernFileTypes', 'IDS_SAVE_FILE_FILTER', 'IDS_OPEN_TEMPLATE_FILTER', 'IDS_OPEN_CSS_FILTER', 'Outcome::Failed', 'LogFailure')) { if ($allSource -notmatch [regex]::Escape($token)) { throw "HTML modern dialog regression: $token" } }
