...
foreach($legacy in @('SetupSci', 'SetSciStyles', 'ConfigureSourceSpecialCharacterRepresentations', 'UpdateSourceLineNumberMargin', 'DefineMarker', 'FoldAll', 'ExpandFold', 'ShowSourceContextMenu', 'SourceEditorWindowProc')) {
    if($mainSource -match "CMainFrame::$legacy\s*\(" -or $mainHeader -match "\b$legacy\s*\(") { throw "CMainFrame must not retain SourceEditorControl presentation method: $legacy" }
}
