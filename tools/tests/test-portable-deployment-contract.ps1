...
$manager = Text 'src\fbe\PluginManager.cpp'; Has $manager 'Plugins\\plugins.json' 'Plugin manifest location'; Has $manager 'LOAD_WITH_ALTERED_SEARCH_PATH' 'Plugin-local dependency lookup'; Has $manager 'REGDB_E_CLASSNOTREG' 'Unknown plugin CLSID rejected locally'; Lacks $manager 'CreateInstance(clsid)' 'No registry COM fallback'; Lacks $manager 'RegEnumKeyEx' 'No registry enumeration'; Lacks $manager 'U::GetProgDirFile(plugin->module)' 'Plugin lookup from program root'
