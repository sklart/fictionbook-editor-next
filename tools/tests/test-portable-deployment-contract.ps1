...
Has $frame 'g_pluginManager.CreateInstance' 'Bundled plugin local-first activation'
Has $frame 'g_pluginManager.DiscoverBundledPlugins' 'Plugin manifest catalog'; Lacks $frame 'DiscoverLegacyPlugins' 'Registry plugins disabled'; Lacks $frame 'RegEnumKeyEx(pluginsKey' 'Registry discovery must not live in CMainFrame'; Lacks $frame 'static const Entry entries[]' 'Hardcoded plugin catalog'
$manager = Text 'src\fbe\plugins\PluginManager.cpp'; Has $manager 'Plugins\\plugins.json' 'Plugin manifest location'; Has $manager 'LOAD_WITH_ALTERED_SEARCH_PATH' 'Plugin-local dependency lookup'; Has $manager 'REGDB_E_CLASSNOTREG' 'Unknown plugin CLSID rejected locally'; Lacks $manager 'CreateInstance(clsid)' 'No registry COM fallback'; Lacks $manager 'RegEnumKeyEx' 'No registry enumeration'; Lacks $manager 'U::GetProgDirFile(plugin->module)' 'Plugin lookup from program root'
