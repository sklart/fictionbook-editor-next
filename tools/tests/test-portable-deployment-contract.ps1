...
Has $frame 'g_pluginManager.DiscoverBundledPlugins' 'Plugin manifest catalog'; Has $frame 'g_pluginManager.DiscoverLegacyPlugins' 'Installed legacy discovery gate'; Lacks $frame 'RegEnumKeyEx(pluginsKey' 'Registry discovery must not live in CMainFrame'; Lacks $frame 'static const Entry entries[]' 'Hardcoded plugin catalog'
$manager = Text 'src\fbe\PluginManager.cpp'; Has $manager 'Plugins\\plugins.json' 'Plugin manifest location'; Has $manager 'LOAD_WITH_ALTERED_SEARCH_PATH' 'Plugin-local dependency lookup'; Lacks $manager 'U::GetProgDirFile(plugin->module)' 'Plugin lookup from program root'
