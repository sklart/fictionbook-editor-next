...
Has $frame 'BundledPluginCatalog' 'Plugin manifest catalog'; Has $frame 'Plugins\\plugins.json' 'Plugin manifest location'; Has $frame 'modulePath = directory + entry.module' 'Plugin module path relative to manifest'; Has $frame 'LOAD_WITH_ALTERED_SEARCH_PATH' 'Plugin-local dependency lookup'; Lacks $frame 'U::GetProgDirFile(plugin->module)' 'Plugin lookup from program root'; Lacks $frame 'static const Entry entries[]' 'Hardcoded plugin catalog'
