...
if($automatic -notmatch 'RECOVERY_TYPING_DEBOUNCE_MS - sinceEdit' -or $automatic -notmatch 'SetTimer\(RECOVERY_TIMER_ID, \(std::max\)\(1U, remaining\)\)' -or ([regex]::Matches($automatic, 'SetTimer\(RECOVERY_TIMER_ID, RECOVERY_INTERVAL_MS\)')).Count -lt 2) { throw 'Deferred recovery must re-arm for the remaining debounce and restore the ordinary interval.' }
