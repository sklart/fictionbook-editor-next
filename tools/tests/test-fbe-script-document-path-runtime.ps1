...
try {
    ...
    $unc = '\\fbe-runtime-server\share\книга-unc.fb2'
    $longDirectory = $root
    while ($longDirectory.Length -le 260) { $longDirectory = Join-Path $longDirectory 'длинный-сегмент-0123456789' }
    $longPath = '\\?\' + (Join-Path $longDirectory 'книга-длинный-путь.fb2')
    ...
    $oldMode,$oldScenario,$oldFirst,$oldSecond,$oldSave,$oldUnc,$oldLong = $env:FBE_NEXT_TEST_MODE,$env:FBE_NEXT_TEST_SCENARIO,$env:FBE_NEXT_TEST_DOCUMENT_PATH_FIRST,$env:FBE_NEXT_TEST_DOCUMENT_PATH_SECOND,$env:FBE_NEXT_TEST_SAVE_PATH,$env:FBE_NEXT_TEST_DOCUMENT_PATH_UNC,$env:FBE_NEXT_TEST_DOCUMENT_PATH_LONG
    try {
        $env:FBE_NEXT_TEST_MODE = '1'; $env:FBE_NEXT_TEST_SCENARIO = 'script-document-path-runtime'; $env:FBE_NEXT_TEST_DOCUMENT_PATH_FIRST = $first; $env:FBE_NEXT_TEST_DOCUMENT_PATH_SECOND = $second; $env:FBE_NEXT_TEST_SAVE_PATH = $saveAs; $env:FBE_NEXT_TEST_DOCUMENT_PATH_UNC = $unc; $env:FBE_NEXT_TEST_DOCUMENT_PATH_LONG = $longPath
    ...
    finally { $env:FBE_NEXT_TEST_MODE,$env:FBE_NEXT_TEST_SCENARIO,$env:FBE_NEXT_TEST_DOCUMENT_PATH_FIRST,$env:FBE_NEXT_TEST_DOCUMENT_PATH_SECOND,$env:FBE_NEXT_TEST_SAVE_PATH,$env:FBE_NEXT_TEST_DOCUMENT_PATH_UNC,$env:FBE_NEXT_TEST_DOCUMENT_PATH_LONG = $oldMode,$oldScenario,$oldFirst,$oldSecond,$oldSave,$oldUnc,$oldLong }
    $values = @{}; foreach($line in Get-Content -LiteralPath $report) { $pair = $line -split '=',2; if($pair.Count -eq 2) { $values[$pair[0]] = $pair[1] } }
    foreach($key in @('unsaved','opened','save_as','other_opened','unicode','unc','long_path')) { if($values[$key] -ne '1') { throw "window.external document-path runtime check failed: $key (value '$($values[$key])')." } }
