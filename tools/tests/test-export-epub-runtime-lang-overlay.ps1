...
$runtimeCppPath = Join-Path $RepositoryRoot 'src\export-epub\RuntimeLocalization.cpp'
$dllMainPath = Join-Path $RepositoryRoot 'src\export-epub\dllmain.cpp'
$resourcePath = Join-Path $RepositoryRoot 'src\export-epub\resource.h'
...
if ($dllMain -notmatch 'InitExportEpubRuntimeStrings\([^)]*\);') {
    throw 'ExportEPUB не вызывает InitExportEpubRuntimeStrings() при загрузке DLL.'
}

$bindingMatches = [regex]::Matches($runtimeCpp, '\{\s*(IDS_[A-Z0-9_]+)\s*,\s*L"([^"]+)"\s*\}')
if ($bindingMatches.Count -eq 0) {
    throw 'В RuntimeLocalization.cpp не найдены runtime JSON binding-строки ExportEPUB.'
}
...
foreach ($match in $bindingMatches) {
    ...
    if ($bindings.ContainsKey($resourceId)) {
        throw "Дублирующий ExportEPUB runtime binding для $resourceId."
    }
    if ($key -notlike 'export_epub.*') {
        throw "ExportEPUB runtime binding $resourceId ссылается на не-ExportEPUB ключ: $key"
    }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("fbe-export-epub-runtime-overlay-" + $PID)
...
try {
    ...
    foreach ($language in $languages) {
        $jsonPath = Join-Path (Join-Path $langRoot $language) 'export-epub.json'
    ...
    Write-Host 'ExportEPUB runtime JSON-overlay прошёл проверку.'
