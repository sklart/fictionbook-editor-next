...
param(
    ...
    [ValidateSet('all','open','new','reload-success','reload-failure','failed-open')][string]$Scenario = 'all'
)
...
try {
    $first = Join-Path $root 'first.fb2'; $second = Join-Path $root 'second.fb2'; $missing = Join-Path $root 'missing.fb2'
    ...
    function Invoke-LifecycleScenario([string]$scenario, [string[]]$required, [bool]$enableTrace = $false) {
        ...
        $oldMode,$oldScenario,$oldOpen,$oldFailed,$oldTrace=$env:FBE_NEXT_TEST_MODE,$env:FBE_NEXT_TEST_SCENARIO,$env:FBE_NEXT_TEST_SUCCESSFUL_OPEN_PATH,$env:FBE_NEXT_TEST_FAILED_OPEN_PATH,$env:FBE_NEXT_TRACE
        try {
            $env:FBE_NEXT_TEST_MODE='1'; $env:FBE_NEXT_TEST_SCENARIO=$scenario; $env:FBE_NEXT_TEST_SUCCESSFUL_OPEN_PATH=$second
			$env:FBE_NEXT_TEST_FAILED_OPEN_PATH=$missing
        ...
        } finally { $env:FBE_NEXT_TEST_MODE,$env:FBE_NEXT_TEST_SCENARIO,$env:FBE_NEXT_TEST_SUCCESSFUL_OPEN_PATH,$env:FBE_NEXT_TEST_FAILED_OPEN_PATH,$env:FBE_NEXT_TRACE=$oldMode,$oldScenario,$oldOpen,$oldFailed,$oldTrace }
    }
    foreach($iteration in 1..$Iterations) {
        ...
        if($Scenario -in @('all','failed-open')) { Invoke-LifecycleScenario 'failed-open-runtime' @('failed=1','identity=1','active=1','session=1','mru_unchanged=1') }
    }
