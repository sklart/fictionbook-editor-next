...
try { & $exe $dll; if ($LASTEXITCODE -ne 0) { throw "ExportEPUB v2 runtime harness failed: $LASTEXITCODE" } } finally { Remove-Item -LiteralPath $exe -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath ($exe -replace '\.exe$','.pdb') -Force -ErrorAction SilentlyContinue }
