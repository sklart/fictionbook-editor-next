# Проверяет, что runtime JSON-overlay ImportEPUB синхронизирован с каталогом Lang и ресурсными ID.
[CmdletBinding()]
param(
...
$runtimeCppPath = Join-Path $RepositoryRoot 'src\import-epub\RuntimeLocalization.cpp'
$dllMainPath = Join-Path $RepositoryRoot 'src\import-epub\dllmain.cpp'
$resourcePath = Join-Path $RepositoryRoot 'src\import-epub\resource.h'
...
if ($dllMain -notmatch 'InitImportEpubRuntimeStrings\(\);') {
    throw 'ImportEPUB не вызывает InitImportEpubRuntimeStrings() при загрузке DLL.'
}

$bindingMatches = [regex]::Matches($runtimeCpp, '\{\s*(IDS_IMPORT_[A-Z0-9_]+)\s*,\s*L"([^"]+)"\s*\}')
if ($bindingMatches.Count -eq 0) {
    throw 'В RuntimeLocalization.cpp не найдены runtime JSON binding-строки ImportEPUB.'
}
...
foreach ($match in $bindingMatches) {
    ...
    if ($bindings.ContainsKey($resourceId)) {
        throw "Дублирующий ImportEPUB runtime binding для $resourceId."
    }
    if ($key -notlike 'import_epub.*') {
        throw "ImportEPUB runtime binding $resourceId ссылается на не-ImportEPUB ключ: $key"
    }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("fbe-import-epub-runtime-overlay-" + $PID)
...
try {
    ...
    foreach ($language in $languages) {
        $jsonPath = Join-Path (Join-Path $langRoot $language) 'import-epub.json'
    ...
    Write-Host 'ImportEPUB runtime JSON-overlay прошёл проверку.'
