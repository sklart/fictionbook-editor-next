...
foreach($token in @('struct SelectionContext', 'insideCode', 'insideSpan', 'm_selection_context', 'InvalidateSelectionContext()', 'RebuildSelectionContext()')) {
    if($header.IndexOf($token, [StringComparison]::Ordinal) -lt 0) { throw "Selection context declaration is missing '$token'." }
}
foreach($token in @('m_selection_context.valid ||', 'SelectionContainer()', 'for (MSHTML::IHTMLElementPtr current', 'TableSelectionAnchor()', 'm_selection_context.tableCell', 'm_selection_context.anchor', 'InvalidateSelectionContext();')) {
...
if($builder.IndexOf('SelectionStructTableCon()', [StringComparison]::Ordinal) -ge 0) { throw 'SelectionContext builder must not perform the legacy table-cell fallback.' }
foreach($token in @('m_selection_context.insideCode', 'm_selection_context.insideSpan')) {
    if($builder.IndexOf($token, [StringComparison]::Ordinal) -lt 0) { throw "SelectionContext builder must derive $token during its ancestor traversal." }
}
foreach($helper in @('SelectionStructCon()', 'SelectionStructImage()', 'SelectionStructSection()', 'SelectionStructTable()', 'SelectionAnchor()')) {
    $pattern = if($helper -eq 'SelectionAnchor()') { '(?<!Table)SelectionAnchor\(\)' } else { [regex]::Escape($helper) }
    if([regex]::IsMatch($builder, $pattern)) { throw "SelectionContext builder must not restart traversal through $helper." }
}
