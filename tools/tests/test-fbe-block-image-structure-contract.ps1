...
foreach ($required in @('fbe-block-image-marker-', 'while(document.getElementById(markerId))', 'InsertBlockImageAtMarker(marker,id)', 'operationError=error', 'if(marker && marker.parentNode) marker.removeNode(true);', 'if(undoStarted) window.external.EndUndoUnit(document);', 'if(!operationError && cleanupError) throw cleanupError;', 'return inserted;')) {
...
foreach ($required in @('MoveFollowingSiblings(source,copy)', 'parent.cloneNode(false)', 'RemoveEmptyInlineFormatting', 'IsMarkerAtParagraphBoundary', 'paragraph.insertAdjacentElement("afterEnd",block)', 'paragraph.insertAdjacentElement("beforeBegin",block)', 'MoveCaretToParagraphStart(rightPart)')) {
