...
if ($entries.Count -ne 132) { throw "Ожидалось 132 строки малых диалогов FBE, получено $($entries.Count)." }
...
foreach($file in $files){
    ...
    foreach($resource in @('IDD_TABLE DIALOGEX','IDD_INPUTBOX DIALOGEX','IDD_ADDIMAGE DIALOGEX','IDD_TOOLS_SETTINGS DIALOGEX','IDD_ABOUTBOX DIALOGEX','IDD_CUSTOMSAVEDLG DIALOGEX','IDD_SETTINGS_WORDS DIALOGEX','IDD_HOTKEYS DIALOGEX','IDD_FIND DIALOGEX','IDD_REPLACE DIALOGEX','IDD_SPELL_CHECK DIALOGEX','IDD_WORDS DIALOGEX','IDD_SETTING_OTHER DIALOGEX','IDD_OPTIONS DIALOGEX','IDC_CHECK_TABLE_TITLE','IDC_ADDIMAGE_ASKAGAIN','IDC_UPDATE','IDC_ENCODING','IDC_CHECK_SHOW_EXCLUSIONS','IDC_BUTTON_HOTKEY_ASSIGN','ID_FIND_NEXT','IDC_REPLACE_ALL','IDC_SPELL_IGNOREALL','IDC_BUTTON_REMOVEHLREPL','IDC_WORDS_FR_BTN_REPL','IDC_DEFAULT_SCRIPTS_FOLDER','IDC_OPTIONS_CLEARIMGS','IDC_SHOWLINENUMBERS','IDC_BACKGROUNDSPELLCHECK')){ if($generatedText -notmatch [regex]::Escape($resource)){ throw "В generated малых диалогов $($file.Language) нет $resource." } }
