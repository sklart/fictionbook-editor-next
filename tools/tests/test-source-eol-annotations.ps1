...
Assert-Contains $mainFrame 'if \(fv\) \{\s*ClearSourceValidationAnnotations\(\);\s*(?:SetValidationStatus\(FBEStatusBar::ValidationStatus::Valid\);\s*)?return 0;' 'annotation clear after successful validation'
