...
foreach($token in @('enum UiDirtyFlags', 'UiDirtySelection', 'UiDirtyClipboard', 'UiDirtySource', 'UiDirtyView', 'UiDirtyToolbar', 'UiDirtyScroll', 'm_ui_dirty', 'InvalidateUi')) {
    if($header.IndexOf($token, [StringComparison]::Ordinal) -lt 0) { throw "UI dirty-state contract is missing '$token'." }
}
foreach($token in @('UiDirtySource | UiDirtyView', 'UiDirtySelection | UiDirtyDocument | UiDirtyView', 'UpdateClipboardCommands()', 'm_ui_dirty = UiDirtyNone')) {
