...
try {
	...
	try { $env:FBE_NEXT_TEST_MODE = '1'; $env:FBE_NEXT_TEST_SCENARIO = 'archive-mru-restart-runtime'; $env:FBE_NEXT_TEST_ARCHIVE_MRU_REOPEN_PATH = $secondCollision; $env:FBE_NEXT_TEST_ARCHIVE_ENTRY = 'book.fb2'; $env:FBE_NEXT_TEST_ARCHIVE_OCCURRENCE = '0'; $process = Start-Process -FilePath $FbeExe -ArgumentList ('--portable -b {0}' -f (Quote $restartReport)) -WorkingDirectory (Split-Path $FbeExe) -PassThru; if(-not $process.WaitForExit($TimeoutSeconds * 1000)) { Stop-Process -Id $process.Id -Force; throw 'Clean MRU restart timed out.' }; $restart = Get-Content -LiteralPath $restartReport -Raw; if($process.ExitCode -ne 0 -or $restart -notmatch 'count=10' -or $restart -notmatch 'menu_count=10' -or $restart -notmatch 'order=1' -or $restart -notmatch 'clean=1' -or $restart -notmatch 'folders=1' -or $restart -notmatch 'reopened=1' -or $restart -notmatch 'localized_nonempty=1' -or $restart -notmatch 'russian_empty=1' -or $restart -notmatch 'english_empty=1') { throw "Clean MRU restart regression failed: $restart" } } finally { foreach($name in $restartVariables) { [Environment]::SetEnvironmentVariable($name, $restartSaved[$name], 'Process') } }
