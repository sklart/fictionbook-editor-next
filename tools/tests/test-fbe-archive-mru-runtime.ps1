...
function Invoke-Mru([string]$archive, [string]$first, [int]$firstOccurrence, [string]$second, [int]$secondOccurrence, [string]$report, [string]$normalEntries = '') {
    ...
    try {
		...
		if ($process.ExitCode -ne 0 -or $state -notmatch 'first=1' -or $state -notmatch 'second=1' -or $state -notmatch 'reopened_first=1' -or $state -notmatch 'missing_entry=1' -or $state -notmatch 'menu_clean=1' -or $state -notmatch 'captions_distinct=1') { throw "Archive MRU runtime failure: $state" }
...
try {
	...
	$restartReport = Join-Path $root 'restart.txt'; $savedScenario = $env:FBE_NEXT_TEST_SCENARIO; $savedPath = $env:FBE_NEXT_TEST_ARCHIVE_MRU_REOPEN_PATH
	try { $env:FBE_NEXT_TEST_MODE = '1'; $env:FBE_NEXT_TEST_SCENARIO = 'archive-mru-restart-runtime'; $env:FBE_NEXT_TEST_ARCHIVE_MRU_REOPEN_PATH = $unicodeZip; $env:FBE_NEXT_TEST_ARCHIVE_ENTRY = 'folder/Книга.fb2'; $env:FBE_NEXT_TEST_ARCHIVE_OCCURRENCE = '0'; $process = Start-Process -FilePath $FbeExe -ArgumentList ('--portable -b {0}' -f (Quote $restartReport)) -WorkingDirectory (Split-Path $FbeExe) -PassThru; if(-not $process.WaitForExit($TimeoutSeconds * 1000)) { Stop-Process -Id $process.Id -Force; throw 'Clean MRU restart timed out.' }; $restart = Get-Content -LiteralPath $restartReport -Raw; if($process.ExitCode -ne 0 -or $restart -notmatch 'count=10' -or $restart -notmatch 'order=1' -or $restart -notmatch 'clean=1' -or $restart -notmatch 'reopened=1') { throw "Clean MRU restart regression failed: $restart" } } finally { $env:FBE_NEXT_TEST_SCENARIO = $savedScenario; $env:FBE_NEXT_TEST_ARCHIVE_MRU_REOPEN_PATH = $savedPath }
