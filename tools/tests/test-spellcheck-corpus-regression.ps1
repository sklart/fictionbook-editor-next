...
function Get-Rejected([string]$Dictionary, [string[]]$Words) {
    $input = [IO.Path]::GetTempFileName()
    try {
        [IO.File]::WriteAllLines($input, $Words, [Text.UTF8Encoding]::new($false))
        return @(& $probe -d $Dictionary -l $input | Where-Object { $_ })
    } finally { Remove-Item -LiteralPath $input -Force -ErrorAction SilentlyContinue }
}
foreach($property in $baseline.languages.psobject.Properties) {
    ...
    $difference = Compare-Object -ReferenceObject @($expectedRejected | Sort-Object) -DifferenceObject @($rejectedWords | Sort-Object)
    if([int]$result.rejected -ne [int]$case.rejected) { throw "$($property.Name): rejected baseline changed: $($result.rejected)" }
    if($difference) { throw "$($property.Name): rejected-word baseline changed: $($difference | Out-String)" }
    foreach($word in @($case.falsePositives)) {
        if($rejectedWords -notcontains $word) { throw "$($property.Name): known false-positive baseline changed: $word" }
    }
    $suggestions = Get-Suggestions $dictionary @($expectedRejected)
    ...
    $benchmark = Get-Result $dictionary $benchmarkWords
