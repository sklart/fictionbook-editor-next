...
if($helpers.Length -eq 0 -or $remove.Length -eq 0) { throw 'Could not extract binary reliability handlers from canonical runtime/main.js.' }
...
try {
    $script = @"
var messages=[]; var ImagesInfo=[]; var rebuilds=0; var fills=0;
...
function Binary(id,type){var b={base64data:'AQID',all:{},parentNode:binobj}; b.all.id=new Input(id); b.all.type=new Input(type); b.all.id.setAttribute('oldId',id); b.all.id.parentNode=b; b.all.type.parentNode=b; b.getElementsByTagName=function(){return [b.all.id,b.all.type];}; b.removeNode=function(){for(var i=0;i<binaries.length;i++)if(binaries[i]===b)binaries.splice(i,1);}; return b;}
...
$helpers
function BuildBinaryControls(binary, fullpath, id, type, data){binary.all.id.value=id; binary.all.type.value=type; binary.all.id.setAttribute('oldId',id); binary.controlIsImage=IsImageBinaryType(type);}
$remove
...
assert(!first.controlIsImage && ImagesInfo.length==1 && ImagesInfo[0].id=='other.jpg', 'image to non-image must remove image controls and cache entry');
first.all.type.value='image/png'; window.event={srcElement:first.all.type}; OnBinaryChange();
assert(first.controlIsImage && ImagesInfo.length==2 && ImagesInfo[0].id=='renamed.jpg', 'non-image to image must restore image controls and cache entry');
