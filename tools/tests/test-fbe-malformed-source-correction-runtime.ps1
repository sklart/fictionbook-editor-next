<# Verifies that corrected malformed Source commits through the application adapter. #>
[CmdletBinding()]
...
try {
    ...
    try {
        $env:FBE_NEXT_TEST_MODE='1'; $env:FBE_NEXT_TEST_SCENARIO='malformed-source-correction-runtime'; $env:FBE_NEXT_TEST_MALFORMED_SOURCE_PATH=$malformed
...
Write-Host 'Malformed source correction runtime regression passed.'
