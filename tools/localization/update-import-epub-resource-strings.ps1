<#
.SYNOPSIS
Обновляет встроенные строковые ресурсы ImportEPUB из JSON-каталога локализации.

.DESCRIPTION
Скрипт читает `localization/plugin-ui/catalog.json`, выбирает строки компонентов
`import-epub.*` с `IDS_*` идентификаторами и генерирует
`src/import-epub/ImportEPUBStrings.generated.rc2`. Файл подключается из
`ImportEPUB.rc`, поэтому окно настроек импорта EPUB и его всплывающие подсказки
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "src\import-epub\ImportEPUBStrings.generated.rc2"
}
...
$entries = @(
    $catalog.strings.PSObject.Properties |
        Where-Object {
            [string]$_.Value.component -like "import-epub.*" -and
...
if ($entries.Count -eq 0) {
    throw "В $CatalogPath не найдены ImportEPUB option/tooltip-строки с resourceId."
}
...
foreach ($language in $languageResources.Keys) {
    if ($declaredLanguages -notcontains $language) {
        throw "В targetLanguages отсутствует язык ImportEPUB: $language"
    }
}

$lines = [Collections.Generic.List[string]]::new()
$lines.Add("// Автоматически сгенерировано из localization/plugin-ui/catalog.json.")
$lines.Add("// Не редактируйте вручную: используйте tools/localization/update-import-epub-resource-strings.ps1.")
$lines.Add("")

foreach ($language in $languageResources.Keys) {
    $resource = $languageResources[$language]
    $lines.Add("/////////////////////////////////////////////////////////////////////////////")
    $lines.Add("// $($resource.Comment) ImportEPUB strings")
    $lines.Add("")
...
if (-not $Quiet) {
    Write-Host "Строковые ресурсы ImportEPUB обновлены из JSON-каталога."
