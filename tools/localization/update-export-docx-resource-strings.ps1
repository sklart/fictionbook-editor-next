<#
.SYNOPSIS
Обновляет встроенные строковые ресурсы ExportDOCX из JSON-каталога локализации.

.DESCRIPTION
Скрипт читает `localization/plugin-ui/catalog.json`, выбирает строки компонентов
`export-docx.*` с `IDR_*`/`IDS_*` идентификаторами и генерирует
`src/export-docx/ExportDOCXStrings.generated.rc2`. Файл подключается из
`ExportDOCX.rc`, поэтому runtime-сообщения и всплывающие подсказки плагина
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $repoRoot "src\export-docx\ExportDOCXStrings.generated.rc2"
}
...
$entries = @(
    $catalog.strings.PSObject.Properties |
        Where-Object {
            [string]$_.Value.component -like "export-docx.*" -and
...
if ($entries.Count -eq 0) {
    throw "В $CatalogPath не найдены ExportDOCX runtime/tooltip-строки с resourceId."
}
...
foreach ($language in $languageResources.Keys) {
    if ($declaredLanguages -notcontains $language) {
        throw "В targetLanguages отсутствует язык ExportDOCX: $language"
    }
}

$lines = [Collections.Generic.List[string]]::new()
$lines.Add("// Автоматически сгенерировано из localization/plugin-ui/catalog.json.")
$lines.Add("// Не редактируйте вручную: используйте tools/localization/update-export-docx-resource-strings.ps1.")
$lines.Add("")

foreach ($language in $languageResources.Keys) {
    $resource = $languageResources[$language]
    $lines.Add("/////////////////////////////////////////////////////////////////////////////")
    $lines.Add("// $($resource.Comment) ExportDOCX strings")
    $lines.Add("")
    ...
    foreach ($entry in $entries) {
        ...
        $lines.Add(("    {0} L""{1}""" -f $resourceId.PadRight(40), $text))
    }
...
if (-not $Quiet) {
    Write-Host "Строковые ресурсы ExportDOCX обновлены из JSON-каталога."
