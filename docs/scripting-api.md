...
успешной записи. При `prompt = true` сначала открывается стандартный диалог с
подтверждением замены существующего файла; при `false` берётся путь из `path`.
Внешний файл при замене полностью восстанавливается из переданных данных.
