...
| FB2 XML autocomplete | Да | Не используется | Внедрён prototype | Единственная компактная таблица предлагает только structural XML: элементы после `<`, допустимые атрибуты после пробела открывающего тега и `href=` после `xlink:`/`l:`. Художественный текст и слова не затрагиваются; полноценный schema-derived vocabulary остаётся следующей задачей. | Не влияет |
