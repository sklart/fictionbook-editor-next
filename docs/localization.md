...
- Основной интерфейс FBE, FBV и все поставляемые плагины используют единый
...
- Команда проверки FB2 в контекстном меню Windows локализуется через MUI-модуль
  ...
  — в `Lang\Shell\FBVVerbResources.dll`. Windows Shell использует ресурсные
  строки вида `@module,-id`, а не JSON.
...
### Мнемоники меню Windows

В строках `MENU` символ `&` перед буквой задаёт стандартную клавишу доступа
Win32 (например, `&File`, `&Файл`, `&Datei`). При переводе его нужно сохранить;
при необходимости перенесите маркер к другой подходящей букве, чтобы избежать
конфликта с соседними пунктами того же меню. Последовательность `&&` означает
обычный символ амперсанда и не является мнемоникой.

`tools/tests/test-fbe-main-menu-mnemonics.ps1` проверяет ровно один маркер у
статических пунктов и уникальность среди непосредственных соседей для всех
поддерживаемых языков. Короткие переводы, в которых все буквенные варианты уже
заняты соседями, допускаются только как точечные документированные исключения
в генераторе; новые конфликты всегда являются ошибкой.
...
Lang\
  ...
  Shell\
    ...
    en-US\
      FBVVerbResources.dll.mui
    ru-RU\
      FBVVerbResources.dll.mui
```

Установщик формирует тот же набор из 12 языков. Английский пакет обязателен
как fallback, русский и украинский отмечены по умолчанию, а другие языки можно
выбрать в отдельной группе компонентов «Языки интерфейса». Runtime JSON и MUI
спутник для команды проверки FB2 ставятся вместе с выбранным языком. Словари
орфографии остаются отдельной группой компонентов.
...
FBE. В настройке всегда доступен пункт `Определяется системой`, а конкретный язык
показывается только если рядом с программой физически установлен
`Lang/<locale>/fbe.json`. Поэтому после удаления языкового каталога вручную он
также исчезает из списка при следующем открытии настроек. При
запуске редактор публикует выбранную локаль двумя способами: в переменную
окружения процесса `FBE_NEXT_UI_LOCALE` и в файл
`%LOCALAPPDATA%\FBE Next\interface-locale.txt`. FBV и плагины сначала читают этот
...
Для основного интерфейса используется исходный каталог
...
- каталог `localization/app-ui` задаёт целевые языки, существующие источники и
  стабильные ключи для генерации `.rc2` и runtime JSON.
...
выбранная локаль используется тем же контрактом, что и для FBV/плагинов.
Внешний слой охватывает строки сообщений, обновлений, статусов, настроек,
hotkey-названий, diagnostics, меню и переведённых диалогов. После применения настроек
...
языке. Открытые модальные окна плагинов намеренно не пересоздаются: язык в
FBE меняется только после закрытия окна настроек, а следующий запуск плагина
уже берёт актуальный пакет `Lang/<locale>`.
...
При переключении только языка редактор не перезапускает Scintilla, MSHTML и
проверку орфографии: тяжёлое применение конфигурации выполняется лишь при
изменении параметров, которые действительно влияют на эти подсистемы.
...
Окно «О программе» применяет runtime JSON для своего заголовка, подписи «Сборка»
и кнопки установки найденного обновления. Статусы проверки обновлений уже были
подключены ранее; поэтому при его открытии все пользовательские тексты этого
сценария согласованы с выбранным языком.
...
маленького custom save dialog. Для его видимых подписей добавлены отдельные
control ID вместо повторяющегося `IDC_STATIC`: `OnInitDialog` накладывает
runtime JSON на «Шаблон XSL», «Включать описание документа» и «Глубина
оглавления», а фильтр выбора XSL/XSLT также берётся из локализованного ресурса.
...
- чтение `FBE_NEXT_UI_LOCALE` и `%LOCALAPPDATA%\FBE Next\interface-locale.txt`;
...
Мастер установки поддерживает те же 12 языков, что и runtime-контур FBE. English,
Русский и Українська отмечены на странице компонентов по умолчанию; English нельзя
снять, потому что он является fallback. Остальные языки устанавливаются по выбору.
Словари проверки орфографии остаются отдельной группой компонентов.

Собственные строки FictionBook Editor Next, не входящие в стандартные страницы
NSIS, извлечены в `localization/installer-ui/catalog.json`. Базовые переводы
хранятся в каталоге, а девять дополнительных европейских языков получают
сгенерированные стартовые тексты из
`localization/installer-ui/european-overrides.json`. Английский fallback
сохраняется для редких технических сообщений, которые ещё не прошли вычитку
носителем языка.
Экспорт можно повторить командой:

```powershell
.\tools\localization\export-nsis-installer-catalog.ps1
```

Контракт проверяет `tools/tests/test-nsis-installer-catalog.ps1`.
...
- MUI-ресурсы FBV/shell-команд в `<locale>\FBVVerbResources.dll.mui`;
...
ресурсами и не должны вручную правиться как источник истины: пока источниками
остаются `localization/app-ui/catalog.json`, `localization/plugin-ui/catalog.json`
и `localization/installer-ui/catalog.json`.
...
- у каждой строки сохраняются стабильный ключ, область (`app-ui`, `plugin-ui`
  или `installer-ui`), переводимый текст, исходная строка `source`,
  `resourceId`, компонент и комментарий для переводчика, если он задан в
  исходном каталоге;
- английский fallback редких технических строк установщика помечается
  `needsTranslation: true`, чтобы не принять его за готовый перевод при
  импорте в Weblate.
...
экспорт во временный каталог и убеждается, что app/plugin/installer-строки
реально попали во все языковые файлы, а английский fallback технических строк
установщика помечен для последующей вычитки.
...
какому языку: ресурсные DLL основного интерфейса, MUI-файлы FBV/shell-команды
в `<locale>\FBVVerbResources.dll.mui`, словари, лицензии, жанры, XSL и будущие
локализованные шаблоны `blank_*.fb2`.

На текущем этапе этот файл используется генератором NSIS-секций языковых
пакетов. Его проверяет `tools/tests/test-language-packs-inventory.ps1`, а
`tools/tests/test-nsis-language-pack-plan.ps1`
...
8. [x] Вынести UI-строки FBE/FBV из старых resource DLL в общий
   JSON→generated pipeline, сохранив встроенный fallback. Последующие пункты
   фиксируют перенос STRINGTABLE, меню и DIALOGEX; runtime JSON-overlay
   используется для всех 12 поддерживаемых языков.
...
51. [x] Завершить инвентаризацию FBE `MENU`/`DIALOGEX`: контрольный запуск
    `tools/localization/analyze-fbe-rc-ui-literals.ps1` показывает 0 ручных
    локализуемых UI-литералов. Все такие элементы проходят через JSON→generated
    pipeline или runtime JSON-overlay, сохраняя встроенный Win32 fallback.
...
Он остаётся вспомогательным экспортом для ревизии структуры; рабочие FBE и
плагины подключают свои generated-фрагменты отдельными генераторами, поэтому
ручные локализуемые `MENU`/`DIALOGEX` уже не остаются в ресурсах FBE.
...
FBE, FBV, ExportHTML, ExportDOCX, ExportEPUB и ImportEPUB используют общий helper `src/common/RuntimeLocalizationCommon.h`. В нём сосредоточены проверка поддерживаемых локалей, чтение выбранного языка из `FBE_NEXT_UI_LOCALE` и `%LOCALAPPDATA%\FBE Next\interface-locale.txt`, UTF-8 JSON-парсер и загрузка `Lang/<locale>/<module>.json`. Порядок fallback теперь один для всех runtime-потребителей: сначала `en-US`, затем выбранная локаль, затем встроенные Win32-ресурсы конкретного модуля. Это убирает дублирование парсера между модулями и делает будущие изменения формата/порядка fallback централизованными.
