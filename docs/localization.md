...
NSIS, дополнительно извлекаются в
`localization/installer-ui/catalog.json`. В нём уже зафиксированы одинаковые
наборы из English, Русский и Українська, а ключи `nsis.*` стабильны для будущей
вычитки и импорта в Weblate. Пока этот каталог является подготовительным:
сборка продолжает использовать действующие `.nsh`, а девять дополнительных
языков по-прежнему получают английский fallback для таких продуктовых строк.
...
- у каждой строки сохраняются стабильный ключ, область (`app-ui`, `plugin-ui`
  ...
  исходном каталоге;
- временный английский fallback строк установщика для девяти ещё не вычитанных
  языков помечается `needsTranslation: true`, чтобы его не принять за готовый
  перевод при импорте в Weblate.
...
реально попали во все языковые файлы, а английский fallback установщика помечен
для последующей вычитки.
