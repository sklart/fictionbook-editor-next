# FictionBook Editor Next

## License and third-party components

FictionBook Editor Next is distributed under GPL-3.0-or-later.  The complete
license text is included in [runtime/gpl-3.0.txt](runtime/gpl-3.0.txt);
[NOTICE](NOTICE) documents the project's FictionBook Editor provenance.

The current inventory of bundled and build-time dependencies is maintained in
[THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md).  Portable archives and the
installer contain this inventory together with the applicable license texts in
`THIRD-PARTY-LICENSES`.

**FictionBook Editor Next** — современная сборка FictionBook Editor для Windows.  
Цель проекта — сохранить привычный редактор FB2, но обновить его окружение: сборку, установщик, shell-интеграцию Windows, проверку FB2, плагины импорта/экспорта и диагностику ошибок.

Проект основан на исходном FictionBook Editor:

- исходный репозиторий SeNS: <https://github.com/sensboston/fictionbookeditor>;
- reference-форк evpobr, использованный как источник отдельных исторических наработок: <https://github.com/evpobr/fictionbookeditor>.

## Что умеет программа

- Открывать, редактировать и сохранять книги FictionBook 2 (`.fb2`).
- Работать с русскими и украинскими ресурсами интерфейса.
- Проверять орфографию через Hunspell.
- Выполнять встроенные скрипты и пользовательские операции над структурой книги.
- Экспортировать книгу в HTML.
- Экспортировать книгу в DOCX и EPUB через подключаемые плагины.
- Импортировать EPUB через подключаемый плагин.
- Запускать проверку FB2 через FictionBook Validator (`FBV.exe`).
- Показывать свойства `.fb2` в Проводнике Windows: автор, название, язык, жанры, серия, версия документа, дата, ключевые слова и идентификатор.
- Показывать миниатюры обложек `.fb2` в Проводнике Windows.
- Устанавливать дополнительные batch-конвертеры для пакетного импорта/экспорта.

## Коротко о терминах

- **FB2 / FictionBook 2** — формат электронных книг на основе XML. Файлы обычно имеют расширение `.fb2`.
- **XML** — текстовый формат с тегами. В FB2 через XML описываются авторы, название, разделы, сноски, картинки и другие части книги.
- **FBE** — FictionBook Editor, основной редактор FB2.
- **FBE Next** — короткое название FictionBook Editor Next, этой обновлённой ветки проекта.
- **FBV / FictionBook Validator** — программа проверки FB2 на ошибки структуры и соответствие схеме FictionBook.
- **Плагин** — дополнительный модуль программы. Например, экспорт в DOCX/EPUB или импорт EPUB.
- **Batch-конвертер** — консольная программа для пакетной обработки из командной строки. Она полезна, когда нужно обработать много файлов без ручного открытия каждого в редакторе.
- **Portable-сборка** — архив, который можно распаковать и запускать без обычной установки. Её изменяемые данные находятся рядом с программой в `Data` (настройки, горячие клавиши, Words, MRU, локаль, диагностика, recovery и пользовательские scripts); она не меняет ассоциации файлов и не регистрирует расширения в Проводнике.
- **Установщик / installer** — обычная установка через `.exe`. Она копирует файлы программы и, если выбрать соответствующие пункты, может добавить интеграцию с Windows.
- **Ассоциация файла** — настройка Windows, какое приложение открывает файл по двойному щелчку. Например, `.fb2` может открываться в FictionBook Editor Next, STDUViewer, SumatraPDF или другой программе.
- **Иконка файла** — значок, который Windows показывает для файлов определённого типа. Он может зависеть от приложения по умолчанию.
- **Shell-интеграция** — интеграция с оболочкой Windows, прежде всего с Проводником: контекстное меню, свойства файла, правая панель сведений, миниатюры.
- **Обработчик свойств / property handler** — компонент, который помогает Проводнику читать из `.fb2` автора, название, жанр, серию и другие свойства.
- **Обработчик миниатюр / thumbnail provider** — компонент, который помогает Проводнику показывать обложку книги как миниатюру файла.
- **Debug-символы / symbols** — технический архив для диагностики падений. Обычному пользователю он обычно не нужен.
- **Smoke-тест** — быстрая автоматическая проверка, что важная функция хотя бы запускается и не ломается сразу.
- **Regression-тест** — проверка, что ранее исправленная ошибка не вернулась.
- **Release-gate** — набор проверок, которые релиз должен пройти перед публикацией.
- **Upstream** — исходный проект или официальный источник зависимости, откуда берутся обновления.
- **WTL** — библиотека для окон и элементов интерфейса Windows. Влияет на внутреннюю основу диалогов, меню и панелей инструментов.
- **Scintilla + Lexilla** — современные компоненты текстового редактора и подсветки кода, используемые в режиме просмотра/редактирования исходного XML.
- **PCRE2** — современный движок регулярных выражений для поиска и замены по шаблонам.
- **Hunspell** — движок проверки орфографии.

## Чем FictionBook Editor Next отличается от исходного FictionBook Editor

### Более современная установка

- Установщик обновлён и собирается автоматически.
- Установка стала чище: основные файлы программы, плагины, shell-интеграция и batch-конвертеры разделены по понятным секциям.
- Интеграция `.fb2` с Windows разделена на отдельные опции: можно не менять приложение по умолчанию и иконку `.fb2`, но при желании добавить только команду проверки или только свойства/миниатюры в Проводнике.
- Системная интеграция Windows устанавливается опционально и требует UAC только тогда, когда это действительно нужно.
- В «Программы и компоненты» Windows записываются версия, издатель, ссылка на проект и примерный размер установки.

<img src="docs/screenshots/FBEditor_programs.png" alt="Запись программы в «Программы и компоненты» Windows" width="70%">

### Улучшенная интеграция с Проводником Windows

- `.fb2` получает нормальные свойства в правой панели сведений и в колонках Проводника.

<img src="docs/screenshots/FB2_properties.png" alt="Свойства в правой панели сведений Проводника" width="70%">

<img src="docs/screenshots/FB2_columns.png" alt="Свойства в колонках Проводника" width="70%">

- Tooltip и панель сведений показывают метаданные книги.

<img src="docs/screenshots/FB2_Tooltip.png" alt="Всплывающая подсказка на файле в Проводнике" width="70%">

- Для книг со встроенной обложкой показываются миниатюры.

<img src="docs/screenshots/FB2_miniatures.png" alt="Миниатюры в Проводнике для книг со встроенной обложкой" width="70%">

- Добавлена диагностика и набор smoke-тестов для проверки shell-регистрации, свойств и миниатюр.

### Проверка FB2 возвращена в исходники

- `FBV.exe` теперь собирается из исходников вместе с проектом.
- Команда проверки `.fb2` добавляется в контекстное меню.
- Название команды локализуется через MUI-ресурсы Windows, для команды проверки добавлена иконка.

<img src="docs/screenshots/FBV_localize.png" alt="Иконка и локализация команды FBV в контекстном меню" width="30%">

### Новые плагины импорта и экспорта

- Добавлен плагин экспорта в DOCX.

<img src="docs/screenshots/ExportDOCX_1.png" alt="Настройки экспорта в DOCX - 1" width="40%"><img src="docs/screenshots/ExportDOCX_2.png" alt="Настройки экспорта в DOCX - 2" width="40%">
<img src="docs/screenshots/ExportDOCX_3.png" alt="Настройки экспорта в DOCX - 3" width="40%"><img src="docs/screenshots/ExportDOCX_4.png" alt="Настройки экспорта в DOCX - 4" width="40%">
- Добавлен плагин экспорта в EPUB.

<img src="docs/screenshots/ExportEPUB.png" alt="Настройки экспорта в EPUB" width="30%">
- Добавлен плагин импорта EPUB.

<img src="docs/screenshots/ImportEPUB.png" alt="Настройки импорта в EPUB" width="30%">

- Добавлены плагины экспорта в DOCX и EPUB, плагин импорта EPUB.

- Для ImportEPUB добавлена вспомогательная библиотека `ImportEPUBLunaSVG.dll`, которая помогает преобразовывать SVG-обложки EPUB в PNG/JPEG.

- Опционально можно установить batch-конвертеры для пакетного запуска импорта/экспорта из командной строки.

<img src="docs/screenshots/ExportDOCXBatch.png" alt="Batch-конвертер ExportDOCXBatch" width="60%">

<img src="docs/screenshots/ExportEPUBBatch.png" alt="Batch-конвертер ExportEPUBBatch" width="60%">

<img src="docs/screenshots/ImportEPUBBatch.png" alt="Batch-конвертер ImportEPUBBatch" width="60%">

### Меньше риска потерять изменения

- Улучшена обработка файлов только для чтения: программа предупреждает пользователя до сохранения и предлагает сохранить копию через «Сохранить как…».

<img src="docs/screenshots/FB2_readonly_save.png" alt="Предупреждение пользователя при сохранении файла только для чтения" width="30%">

- Исправлены сценарии, где относительный путь к файлу из командной строки мог приводить к ошибке открытия.
- Добавлена локальная диагностика сбоев и архив debug-символов для релизов.

### Локализация

- Добавлена машинная локализация на основные европейские языки. Язык теперь переключается "на лету", без перезапуска программы.
<img width="338" height="379" alt="Снимок экрана 2026-07-15 091221" src="https://github.com/user-attachments/assets/398a31a9-7bd1-45e9-8df3-28d80e7375e8" />

- Локализация из ресурсов вынесена в json-файлы, что позволит сторонним переводчикам легко вносить свои правки, а также позднее подключить переводы к веб-инструментам перевода типа Weblate.

### Обновлены зависимости и сборка

- Проект адаптирован для современных версий Visual Studio.
- Обновлён орфографический движок Hunspell: это улучшает совместимость с современными словарями и снижает риск проблем при проверке нестандартных словоформ.
- Старый PCRE заменён на PCRE2: регулярные выражения теперь опираются на поддерживаемый современный движок.
- Исторический SciLexer заменён на актуальную связку Scintilla + Lexilla: source-view получает более современную основу для редактирования и подсветки XML.
- WTL обновлён с исторической версии 8.1 до WTL 10.0.1. Для обычного пользователя это не добавляет отдельную кнопку в интерфейсе, но снижает технический долг вокруг окон, меню, панелей инструментов и диалогов: проект лучше стыкуется с современными ATL/Visual Studio, проще поддерживает текущие версии Windows и получает более безопасную основу для будущих UI-доработок без переписывания интерфейса с нуля.
- Автоматизированы проверки обновлений Scintilla, Lexilla, PCRE2, Hunspell и WTL.
- Добавлены release-gate проверки: сборка, portable-пакет, установщик, ZIP с debug-символами, checksum-файл.

## Установка

В релизах GitHub публикуются:

- `FictionBookEditorNext-<version>-win32-setup.exe` — единый установщик для
  Windows 7 SP1 и новее.
- `FictionBookEditorNext-<version>-win32-portable.zip` — единая portable-сборка
  для Windows 7 SP1 и новее.
- `FictionBookEditorNext-<version>-win32-symbols.zip` — debug-символы для
  диагностики падений.
- `SHA256SUMS.txt` — контрольные суммы артефактов.

Рекомендуемый вариант для большинства пользователей — единый установщик.

В переходных релизах серии 3.0.8 временно могут присутствовать файлы с
`-win7-` в имени. Это не отдельная сборка: это идентичные копии универсальных
артефактов, нужные исключительно для обновления ранее опубликованного
`v3.0.8-rc.1`.

В установщике интеграция с `.fb2` разбита на отдельные пункты. Если вы уже
используете другую читалку и хотите сохранить привычную иконку файлов, не
выбирайте пункт назначения FBE Next приложением для `.fb2`. Можно отдельно
добавить команду «Проверить FB2 на ошибки» или свойства/миниатюры в Проводнике.

## Поддерживаемые версии Windows

FictionBook Editor Next поддерживает Windows 7 SP1, Windows 8.1, Windows 10 и
Windows 11 одним набором Win32-бинарников. Release pipeline фиксирует target
macros и проверяет импорты, чтобы критичные runtime-модули не требовали функций
Windows 8+ напрямую; при этом High DPI, PerMonitorV2 и современные возможности
Scintilla используются там, где они доступны.

Windows 7 должна быть обновлена до SP1 и иметь актуальный Universal CRT /
Visual C++ runtime: отдельные современные компоненты и плагины могут требовать
системные обновления.

Windows XP и Windows Vista не поддерживаются стандартной сборкой Next: часть
обновлённых библиотек, shell-интеграции и tooling ориентирована на более новые
версии Windows. Возможность отдельной legacy-сборки для Windows XP рассматривается
как отдельная задача и потребует осознанного отказа от части современной
функциональности.

## Обновления

Проверка обновлений в окне «О программе» использует файл `update.xml` из этого репозитория.
Обычная установленная копия скачивает доверенный setup из GitHub Releases. Portable-копия не запускает setup автоматически: кнопка обновления открывает portable ZIP той же версии, чтобы не изменить выбранный способ развёртывания и не затронуть `Data`.

## Как сообщить о проблеме

Пожалуйста, используйте [инструкцию по сообщению об ошибках](docs/reporting-issues.md).
В ней перечислено, какие сведения, скриншоты, тестовые книги и файлы дампов
помогают воспроизвести и исправить проблему быстрее.

## Сборка из исходников

Требуется Windows и Visual Studio с C++ workload.

```powershell
git submodule update --init --recursive
.\tools\build\build.ps1 -Configuration Release -Platform Win32 -SkipUpx
.\tools\build\verify-release.ps1 -Configuration Release
.\tools\build\create-release.ps1 -Configuration Release -Platform Win32 -SkipBuild -SkipUpx
```

Готовые файлы будут в `out\artifacts`.

Подробности:

- [docs/building.md](docs/building.md)
- [docs/versioning.md](docs/versioning.md)
- [docs/release-checklist.md](docs/release-checklist.md)
- [docs/manual-test-plan.md](docs/manual-test-plan.md)
- [docs/localization.md](docs/localization.md)

## Документация для разработчиков

- [docs/technical-changes-from-sensboston.md](docs/technical-changes-from-sensboston.md) — техническое описание отличий от исходного проекта.
- [docs/test-contours.md](docs/test-contours.md) — карта автоматических и ручных тестовых контуров.
- [docs/shell-extension-modernization.md](docs/shell-extension-modernization.md) — modern shell-интеграция.
- [docs/upstream-evpobr-analysis.md](docs/upstream-evpobr-analysis.md) — заметки по reference-форку evpobr.

## Лицензия

Проект сохраняет лицензионную модель исходного FictionBook Editor: GNU General Public License v3.  
Тексты лицензии находятся в runtime-файлах проекта.

## Благодарности

Спасибо авторам исходного FictionBook Editor, SeNS, LitRes, Mike Matsnev, авторам скриптов, переводчикам, тестировщикам и всем, кто поддерживал редактор живым. FictionBook Editor Next — это продолжение этой линии, а не попытка стереть историю проекта.
