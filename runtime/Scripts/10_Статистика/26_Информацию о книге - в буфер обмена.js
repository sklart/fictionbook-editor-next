...
// ---------------------------------------------------------------
 ...
 //     Показывать аннотацию
 var ShowAnnotation = 1;      // 0 ; 1 //      ("0" — никогда не показывать, "1" — всегда показывать)
...
//  * 295 жанров.
 ...
 if (Zhanry != "")                                                                       //  Если список с жанрами не пустой...
         Txt2 += " • Жанр книги:\n" + Zhanry + "\n";    //  добавляем его к основному тексту.
     else Txt2 += " •  Жанр книги:\n <не указан>\n\n";   //  Если добавлять нечего, то пишем и об этом.
...
// -----------------------------
 ...
 if (count_Author != 0) {                                                            //  Если в книге есть имена авторов...
         if (count_Author == 1)                                                         //  И если это имя одно...
                 Txt2 += " • Автор книги:\n" + Avtory + "\n";        //  то добавляем в основной текст соответствующую запись.
             else  Txt2 += " • Авторы книги:\n" + Avtory + "\n";  //  А если несколько, то другую запись.
         }
     else Txt2 += " •  Автор книги:\n <не указан>\n\n";   //  Если добавлять нечего, то пишем и об этом.
...
// -----------------------------
 ...
 if (bookTitle != "")                                                     //  Если оно есть...
         Txt2 += " • Название книги:\n " + bookTitle + "\n\n";   //  то добавляем его в основной текст.
     else Txt2 += " •  Название книги:\n <не указано>\n\n";   //  Если добавлять нечего, то пишем и об этом.
 ...
 if (bookLang != ""  &&  bookLang != "ru") {          //  Если язык книги не русский...
         ...
         for (k=0; k<Length; k++)                                          //  Просматриваем известный список кодов.
                 if (mLangCode[k] == bookLang) {                  //  И если язык книги есть в этом списке...
                         Jazyk = " • Язык книги:\n " + mLang2[k];   //  заполняем блок текста расшифрованным кодом
                         break;                                                                          //  и выходим из цикла.
                         }
         if (Jazyk == "")                                                                             //  Если код не найден...
                 Jazyk = " •  Язык книги:\n <" + bookLang + ">";    //  заполняем блок текста   кодом языка.
         }

 if (bookLang == "")                                                        //  Если язык книги не указан...
         Jazyk = " •  Язык книги:\n <не указан>";   //  то заполняем блок текста соответствующей записью.

 if (Jazyk != "")                          //  Если блок текста заполнен...
         Txt2 += Jazyk + "\n\n";   //  то добавляем его к основному тексту.
...
// -----------------------------
 ...
 if (count_Seq_1 != 0) {                                                      //  Если в книге есть серии...
         if (count_Seq_1 == 1)                                                 //  И если эта серия одна...
                 Txt2 += " • Серия:\n" + seqS_1 + "\n";         //  то добавляем в основной текст соответствующую запись.
             else  Txt2 += " • Серии:\n" + seqS_1 + "\n";  //  А если серий несколько, то другую запись.
         }
...
// -----------------------------
 ...
 if (bookDate != "")                                                     //  Если она есть...
         Txt2 += " • Дата создания:\n " + bookDate + "\n\n";   //  то добавляем её в основной текст.
...
// -----------------------------
 ...
 if (Perevod != "")                          //  Если блок перевода заполнен...
         Txt2 += Perevod + "\n";   //  то добавляем его к основному тексту.
...
// -----------------------------
 ...
 if (count_illustrator != 0) {                                                                     //  Если в книге есть имена художников...
         if (count_illustrator == 1)                                                               //  И если это имя одно...
                 Txt2 += " • Художник:\n" + Hudozhniki + "\n";           //  то добавляем в основной текст соответствующую запись.
             else  Txt2 += " • Художники:\n" + Hudozhniki + "\n";    //  А если несколько, то другую запись.
         }
...
// -----------------------------
 ...
 if (Dopolnitelno != "")                                                                                                 //  Если блок дополнительной информации заполнен...
...
// -----------------------------
 ...
 if (Pechatnoe_izdanie != 0)                                                                                   //  Если блок перевода заполнен...
...
// -----------------------------
 ...
 var Annotatsija = "";   //  Текст аннотации.
 ...
 if (ShowAnnotation == 1  &&  fbwBody.firstChild.className == "annotation") {     //  Если первый раздел - это аннотация:
         ...
         for (k=0; k<Length; k++) {                                       //  Просмотр всех строк по очереди.
                 ...
                 if (s != "")                                               //  Если строка не пустая...
                         Annotatsija += ks + s + "\n";                 //  добавляем к аннотации эту строку
                 if (Annotatsija.length > Dlina + 500)  break;    //  и если полученный текст достаточно большой, то выходим из цикла.
                 }
         if (Annotatsija != "")                                                            //  Если аннотация заполнена...
                 Annotatsija = Annotatsija.replace(/\n$/, "");   //  то удаляем последний разрыв строки.
         if (Annotatsija.length > Dlina) {                         //  Если аннотация слишком длинная...
                 s = Annotatsija.replace(re11, re11_);   //  пробуем ее обрезать,
                 if (s != Annotatsija)
                         s = s.replace(re12, re12_);   //  при необходимости добавляем в конце многоточие
                 Annotatsija = s;                            //  и окончательно утверждаем обработанный вариант аннотации.
                 }
         }
...
// -----------------------------
 ...
 if (Oglavlenie != "") {                                         //  Если блок оглавления заполнен...
         Txt2 += " • Оглавление\n ~~~~~~~~~~~~~~~~~~~~\n" + Oglavlenie;   //  то добавляем его к основному тексту.
         if (Dizajn != 1)   //  Если выбран "сжатый" дизайн...
                 Txt2 += "\n";   //  то добавляем дополнительную пустую строку.
         }
...
// -----------------------------
 ...
 if (SektsiiSnosok != "")                          //  Если блок текста для примечаний и комментариев заполнен...
         Txt2 += SektsiiSnosok + "\n";   //  то добавляем его к основному тексту.
...
// -----------------------------
 ...
 mSt[ind]="";  ind++;
                                                              ...
                                                              mSt[ind]=Mark(count_Genre) +           "Жанр книги .  .  .  .  .  .  .	" + Counter(count_Genre);  ind++;
                                                              mSt[ind]=Mark(bookTitle) +                  "Название книги   .  .  .  .	" + Counter(bookTitle);  ind++;
                                                              mSt[ind]=Mark(count_Author) +         "Авторы книги .  .  .  .  .  .	" + Counter(count_Author);  ind++;
                                                              mSt[ind]=Mark(bookLang) +                 "Язык книги  .  .  .  .  .  .  .	" + Counter(bookLang);  ind++;
 if (count_Seq_1 != d)                 { mSt[ind]=Mark(count_Seq_1) +            "Серия книги   .  .  .  .  .  .	" + Counter(count_Seq_1);  ind++ }
 ...
 if (ShowAnnotation == 1  &&  Annotatsija !=""  ||  d) {
                                                              mSt[ind]=Mark(Annotatsija) +              "Аннотация   .  .  .  .  .  .  .	" + Counter(Annotatsija);  ind++ }
