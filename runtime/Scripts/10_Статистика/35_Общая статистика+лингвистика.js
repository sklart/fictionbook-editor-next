//  Скрипт "Общая статистика + лингвистика"

// Автор скрипта - Александр Ка

// Доработка - DeepSeek, TaKir (добавлена "лингвистика")

// v.1.4 (TaKir Mod)  — добавление минимального лингво-анализа (26.11.2025)
//  определение языков, подсчет частей речи, "словарного запаса".
...
function Run() {
 ...
 var ScriptName="«Общая статистика + лингвистика»";
 var NumerusVersion="1.4* (TaKir-Mod)";
...
// НОВЫЕ ФУНКЦИИ ЛИНГВИСТИЧЕСКОГО АНАЛИЗА
...
function detectPartOfSpeech(word) {
    if (word.length <= 2) {
        if (/(и|а|но|или|что|чтобы|как|когда|если|да|же|ли|либо|ни)/.test(word)) {
            partsOfSpeech.conjunctions++; return;
        }
        if (/(в|на|за|под|над|от|до|из|у|о|об|со|по|про|при|без|для|к|с)/.test(word)) {
            partsOfSpeech.prepositions++; return;
        }
    }
    if (/(ть|ти|ться|тся|лся|лась|лось|лись)$/.test(word)) {
        partsOfSpeech.verbs++; return;
    }
    if (/(ый|ий|ой|ая|яя|ое|ее)$/.test(word)) {
        partsOfSpeech.adjectives++; return;
    }
    partsOfSpeech.nouns++;
}

// УЛУЧШЕННАЯ ФУНКЦИЯ ОПРЕДЕЛЕНИЯ ЯЗЫКОВ ДЛЯ СМЕШАННЫХ ТЕКСТОВ
function detectLanguage(word) {
    // Сначала проверяем явные признаки языков
    
    // Французский - диакритика и частотные слова
    if (/[àâäèéêëîïôöùûüÿçœæ]/.test(word) || 
        /^(le|la|les|de|des|du|et|est|que|dans|une|il|elle|on|nous|vous|ils|elles|je|tu|il|elle|on|nous|vous|ils|elles|mon|ton|son|ma|ta|sa|mes|tes|ses|notre|votre|leur|nos|vos|leurs|ce|cet|cette|ces)$/.test(word)) {
        languageStats.french++;
        return;
    }
    
    // Немецкий - умлауты и артикли
    if (/[äöüß]/.test(word) || 
        /^(der|die|das|den|dem|des|ein|eine|einer|einem|einen|und|ist|sind|war|waren|bin|bist|ist|sind|seid|sind|habe|hast|hat|haben|habt|haben)$/.test(word)) {
    ...
    if (/[ñáéíóúü¿¡]/.test(word) || 
        /^(el|la|los|las|un|una|unos|unas|y|o|pero|porque|como|cuando|donde|quien|que|es|soy|eres|es|somos|sois|son)$/.test(word)) {
    ...
    if (/[àèéìíîòóù]/.test(word) ||
        /^(il|lo|la|i|gli|le|un|uno|una|e|o|ma|perché|come|quando|dove|chi|che|è|sono|sei|è|siamo|siete|sono)$/.test(word)) {
...
// ИСПРАВЛЕННАЯ ФУНКЦИЯ ОПРЕДЕЛЕНИЯ ЧАСТЕЙ РЕЧИ
function detectPartOfSpeech(word) {
    // Местоимения (расширенный список)
    if (/(^я$|^ты$|^он$|^она$|^оно$|^мы$|^вы$|^они$|^мой$|^твой$|^свой$|^наш$|^ваш$|^его$|^её$|^их$|^кто$|^что$|^какой$|^чей$|^который$|^сколько$)/.test(word)) {
    ...
    // Числительные (простые случаи)
    if (/(^один$|^два$|^три$|^четыре$|^пять$|^шесть$|^семь$|^восемь$|^девять$|^десять$|^сто$|^тысяча$|^миллион$|^первый$|^второй$|^третий$)/.test(word)) {
        partsOfSpeech.numerals++;
        return;
    }
    
    // Наречия (расширенные окончания)
    if (/(о$|е$|ко$|но$|то$|где$|куда$|когда$|почему$|зачем$|как$|сколько$|хорошо$|плохо$|быстро$|медленно$)/.test(word)) {
        partsOfSpeech.adverbs++;
        return;
    }
    
    // Глаголы (расширенные окончания)
    if (/(ть$|ти$|ться$|тся$|лся$|лась$|лось$|лись$|ет$|ут$|ют$|ат$|ят$|ите$|йте$|ал$|ала$|ало$|али$)/.test(word)) {
        partsOfSpeech.verbs++;
        return;
    }
    
    // Прилагательные (расширенные окончания)
    if (/(ый$|ий$|ой$|ая$|яя$|ое$|ее$|ые$|ие$|ого$|ему$|ой$|ую$|ым$|ом$|ская$|ское$|ской$)/.test(word)) {
        partsOfSpeech.adjectives++;
        return;
    }
    
    // Всё остальное - существительные
    partsOfSpeech.nouns++;
}
...
// НОВЫЕ ПЕРЕМЕННЫЕ ДЛЯ ЛИНГВИСТИЧЕСКОГО АНАЛИЗА
...
var partsOfSpeech = {
    ...
    "arabic_numbers": 0, "roman_numbers": 0  // ДОБАВИЛИ
};
var reWordLing = /[А-Яа-яA-Za-zЁё0-9]+/g;
...
// Поиск и подсчет пустых строк
   ...
   // НОВЫЙ ПОДСЧЕТ: символы без пробелов и лингвистический анализ
   var cleanText = sct.replace(/^\s+|\s+$/g, "");
   totalCharsWithoutSpaces += cleanText.replace(/\s/g, "").length;
   
   var words = sct.match(reWordLing);
   if (words) {
       for (var w = 0; w < words.length; w++) {
           ...
           // Остальной код для обычных слов...
           if (uniqueWords[word]) {
               uniqueWords[word]++;
               if (uniqueWords[word] === 2) duplicateWords++;
           } else {
               uniqueWords[word] = 1;
           }
           
           var lemma = getLemma(word);
           if (vocabulary[lemma]) vocabulary[lemma]++;
           else vocabulary[lemma] = 1;
...
for (var word in uniqueWords) {
    if (uniqueWords.hasOwnProperty(word)) uniqueWordCount++;
}
var vocabularySize = 0;
for (var lemma in vocabulary) {
    if (vocabulary.hasOwnProperty(lemma)) vocabularySize++;
}
...
// НОВАЯ СТАТИСТИКА
 mSt[ind]="Символов (с пробелами)  .  .  .  .  .  .  .  " + count_littera;  ind++;
 mSt[ind]="Символов (без пробелов) .  .  .  .  .  .  . " + totalCharsWithoutSpaces;  ind++;
 mSt[ind]="Строк   .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+count_P;  ind++;
 mSt[ind]="Слов .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+count_Word;  ind++;
 mSt[ind]="Уникальных слов  .  .  .  .  .  .  .  .  .  .  .  .	"+uniqueWordCount;  ind++;
 mSt[ind]="Словарный запас  .  .  .  .  .  .  .  .  .  .  .  .	"+vocabularySize;  ind++;
 mSt[ind]="Повторяющихся слов  .  .  .  .  .  .  .  .	"+duplicateWords;  ind++;
 mSt[ind]="";  ind++;

  mSt[ind]="• ИСПОЛЬЗУЕМЫЕ ЯЗЫКИ:";  ind++;
 mSt[ind]="Русский  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+russianPercent+"%  ("+languageStats.russian+" слов)";  ind++;
 mSt[ind]="Английский  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+englishPercent+"%  ("+languageStats.english+" слов)";  ind++;
if (languageStats.french > 0)  { mSt[ind]="Французский  .  .  .  .  .  .  .  .  .  .  .  .  .	"+frenchPercent+"%  ("+languageStats.french+" слов)";  ind++; }
if (languageStats.german > 0)  { mSt[ind]="Немецкий  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+germanPercent+"%  ("+languageStats.german+" слов)";  ind++; }
if (languageStats.spanish > 0) { mSt[ind]="Испанский  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+spanishPercent+"%  ("+languageStats.spanish+" слов)";  ind++; }
if (languageStats.italian > 0) { mSt[ind]="Итальянский  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+italianPercent+"%  ("+languageStats.italian+" слов)";  ind++; }
if (languageStats.other > 0)   { mSt[ind]="Другие  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+otherPercent+"%  ("+languageStats.other+" слов)";  ind++; }
 mSt[ind]="";  ind++;

 mSt[ind]="• ОСНОВНЫЕ ЧАСТИ РЕЧИ:";  ind++;
 mSt[ind]="Существительные  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.nouns;  ind++;
 mSt[ind]="Прилагательные  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.adjectives;  ind++;
 mSt[ind]="Глаголы  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.verbs;  ind++;
 mSt[ind]="Наречия  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.adverbs;  ind++;
 mSt[ind]="Местоимения  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.pronouns;  ind++;
 mSt[ind]="Числительные  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.numerals;  ind++;
 mSt[ind]="Предлоги  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.prepositions;  ind++;
 mSt[ind]="Союзы  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.conjunctions;  ind++;
 mSt[ind]="Арабские цифры  .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.arabic_numbers;  ind++;
 mSt[ind]="Римские цифры .  .  .  .  .  .  .  .  .  .	"+partsOfSpeech.roman_numbers;  ind++;
 mSt[ind]="";  ind++;
...
// -----------------------------
 ...
 if (SaveStat == 1  &&  AskYesNo ("◊  Сохранить текст общей статистики       \n               в буфер обмена?"))
         ...
         //  сохранение в файл
 var name="Общая статистика + лингвистика ("+currentDate+" "+currentHours+"-"+currentMinutes+"-"+currentSeconds+").txt";
