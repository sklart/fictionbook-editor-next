// Скрипт "Создать эпиграфы из жирно-курсивных абзацев после заголовков" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для создания эпиграфов в fb2 документах
// из полностью жирно-курсивных абзацев (содержащих совмещение STRONG+EM или EM+STRONG),
// следующих сразу после заголовков.
...
// Абзацы только с жирностью или только с курсивом исключаются из обработки.
// Только совмещение жирности и курсива (STRONG+EM или EM+STRONG) обрабатывается.
...
// Опциональное расформатирование созданных эпиграфов от исходной жирности и курсива.
...
function Run() {
    ...
    // Размечать эпиграфом абзацы, похожие на диалоги
    // 0 - нет, 1 - да
    var formatDialogs = 0;
    ...
    // Расформатировать созданные эпиграфы от жирности и курсива
    // 0 - оставить как есть, 1 - расформатировать от жирности и курсива
    ...
    var scriptName = "Создать эпиграфы из жирно-курсивных абзацев после заголовков";
    var version = "1.0";
    ...
    // Проверка: полностью ли элемент внутри STRONG+EM или EM+STRONG
    function isFullyStrongEm(element) {
        if (!element) return false;
        var html = element.innerHTML;
        if (!html) return false;
        html = trimStr(html);

        // Вариант 1: <STRONG><EM>...</EM></STRONG>
        var pattern1 = /^<STRONG(\s[^>]*)?><EM(\s[^>]*)?>.*<\/EM><\/STRONG>$/i;
        // Вариант 2: <EM><STRONG>...</STRONG></EM>
        var pattern2 = /^<EM(\s[^>]*)?><STRONG(\s[^>]*)?>.*<\/STRONG><\/EM>$/i;
    ...
    // Удаление тегов STRONG и EM из элемента
    function stripStrongEm(element) {
        if (!element) return;
        var html = element.innerHTML;
        if (!html) return;
    ...
    function findEpigraphCandidates(bodyElement) {
        ...
        for (var i = 0; i < allSections.length; i++) {
            ...
            // Только совмещение, не чистый STRONG и не чистый EM
            if (!isFullyStrongEm(firstP)) continue;
    ...
    if (showStatistics === 1) {
        var resultMsg = scriptName + "\n" +
            ...
            "✓ Расформатировано от жирности и курсива: " + totalReformatted + " абз.\n\n" +
