// Скрипт "Создать эпиграфы из курсивных (НЕ-жирных) абзацев после заголовков" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для создания эпиграфов в fb2 документах
// из полностью курсивных абзацев (не содержащих жирность), следующих сразу после заголовков.
...
// Абзацы с совмещением курсива и жирности (EM+STRONG или STRONG+EM) исключаются из обработки.
...
// Опциональное расформатирование созданных эпиграфов от исходного курсива.
...
function Run() {
    ...
    // Расформатировать созданные эпиграфы от курсива
    // 0 - оставить как есть, 1 - расформатировать от курсива
    ...
    var scriptName = "Создать эпиграфы из курсивных (НЕ-жирных) абзацев после заголовков";
    var version = "1.0";
    ...
    // Проверка: полностью ли элемент внутри EM (без STRONG внутри)
    function isFullyEm(element) {
        if (!element) return false;
        var html = element.innerHTML;
        if (!html) return false;
        html = trimStr(html);

        var startPattern = /^<EM(\s[^>]*)?>/i;
        var endPattern = /<\/EM>$/i;
        ...
        if (/<STRONG(\s[^>]*)?>/i.test(innerHtml) || /<\/STRONG>/i.test(innerHtml)) {
            return false;
        }
    ...
    if (showStatistics === 1) {
        var resultMsg = scriptName + "\n" +
            ...
            "✓ Расформатировано от курсива: " + totalReformatted + " абз.\n\n" +
