//======================================
//             «Генеральная уборка» (версия 2.10) ИНОСТРАНКА
//                                             Engine by ©Sclex
//                                                    01.05.2007 – 31.08.2022


//v.2.10 — TaKir (18.09.2026) для обработки англоязычных и немецкоязычных текстов.

//отключена обработка кавычек:
// (re05, re27, re32, re33, re34, re36, re57, re154, re155) для корректной работы с англоязычными и немецкими текстами
// изменены (не отключены): re03 (убран » из класса), re04 (убран « из класса)
//отключена обработка апострофов:
// (re94, re162)
//отключены из-за ложных срабатываний: re143 (комбинируемое бреве+и), re160 (символ U+E098)
//======================================
...
//======================================
//v.2.7 — регэкспы (п.п. 142-149) — TaKir
//======================================
//v.2.8 — регэкспы (п.п. 150-156) — TaKir
//======================================
//v.2.9 — регэкспы (п.п. 157-165) — TaKir
//======================================
//v.2.10 — отключена обработка кавычек для текстов с обратными кавычками:
// (re05, re27, re32, re33, re34, re36, re57, re154, re155) для корректной работы с англоязычными и немецкими текстами
//======================================

var VersionNumber="2.10";
...
// замена знаков "Минус" U+2212 на длинное тире (TaKir)
 var re150 = new RegExp("([\\u2212])","gi");
 var re150_ = "—";
 var count_150 = 0;


// замена знаков "Идеограмма 一" U+4E00 на длинное тире (TaKir)
 var re151 = new RegExp("([\\u4E00])","gi");
 var re151_ = "—";
 var count_151 = 0;
 
// замена знаков "Полноширинная левая скобка" U+FF08 на левую круглую скобку (TaKir)
 var re152 = new RegExp("([\\uFF08])","gi");
 var re152_ = "(";
 var count_152 = 0; 
 
// замена знаков "Полноширинная правая скобка" U+FF09 на правую круглую скобку (TaKir)
 var re153 = new RegExp("([\\uFF09])","gi");
 var re153_ = ")";
 var count_153 = 0;
 
// замена знаков "Левая двойная угловая скобка" U+300A на левую кавычку-елочку (TaKir)
// var re154 = new RegExp("([\\u300A])","gi");
// var re154_ = "«";
// var count_154 = 0;
 
// замена знаков "Правая двойная угловая скобка" U+300B на правую кавычку-елочку (TaKir)
// var re155 = new RegExp("([\\u300B])","gi");
// var re155_ = "»";
// var count_155 = 0;
 
 
// удаление знаков "Метка справа-налево" U+200F (TaKir)
 var re156 = new RegExp("([\\u200F])","gi");
 var re156_ = "";
 var count_156 = 0;


// удаление знаков "Селекторы начертания (1-16)" (TaKir)
 var re157 = new RegExp("([\\uFE00-\\uFE0F])","gi");
 var re157_ = "";
 var count_157 = 0;


// удаление знаков "Стрелка влево с крючком U+21A9" (TaKir)
 var re158 = new RegExp("([\\u21A9])","gi");
 var re158_ = "";
 var count_158 = 0;


// замена кавычек в названии издательства (TaKir)
 var re159 = new RegExp("(«Издательство «)([^»]+)(»)", "gi");
 var re159_ = "«Издательство „$2“»";
 var count_159 = 0;


// замена символа приватной зоны U+E098 на ударение для предшествующей гласной, только внутри слова (TaKir)
 var re160 = new RegExp("([аеёиоуыэюя]\\uE098)(?=[а-яё])","gi");
 var re160_ = "$1́";
 var count_160 = 0;


// замена ошибочно переданных ударений в виде цифры "2" после строчной гласной, только внутри слова (TaKir)
 var re161 = new RegExp("([аеёиоуыэюя])2(?=[а-яё])","gi");
 var re161_ = "$1$2́";
 var count_161 = 0;
 
 
// замена ошибочных ударений в виде апострофа после строчной гласной, только внутри слова (TaKir)
//  var re162 = new RegExp("([а-яё])([аеёиоуыэюя])'(?=[а-яё])","gi");
//  var re162_ = "$1$2́";
//  var count_162 = 0;
 
 
// замена знаков "Полноширинный дефис-минус" U+FF0D на длинное тире (TaKir)
 var re163 = new RegExp("([\\uFF0D])","gi");
 var re163_ = "—";
 var count_163 = 0;


// удаление знаков "Область для частного использования U+F06E" (TaKir)
 var re164 = new RegExp("([\\uF06E])","gi");
 var re164_ = "";
 var count_164 = 0;
 
 
// узамена знаков "Вопросительный восклицательный знаки U+2048" (TaKir)
 var re165 = new RegExp("([\\u2048])","gi");
 var re165_ = "?!";
 var count_165 = 0;
...
// 	   if (s.search(re143)!=-1)       { count_143+=s.match(re143).length;s=s.replace(re143, re143_); }
	   ...
	   if (s.search(re151)!=-1)       { count_151+=s.match(re151).length;s=s.replace(re151, re151_); }
	   if (s.search(re152)!=-1)       { count_152+=s.match(re152).length;s=s.replace(re152, re152_); }
	   if (s.search(re153)!=-1)       { count_153+=s.match(re153).length;s=s.replace(re153, re153_); }
//	   if (s.search(re154)!=-1)       { count_154+=s.match(re154).length;s=s.replace(re154, re154_); }
//	   if (s.search(re155)!=-1)       { count_155+=s.match(re155).length;s=s.replace(re155, re155_); }
	   if (s.search(re156)!=-1)       { count_156+=s.match(re156).length;s=s.replace(re156, re156_); }
	   if (s.search(re157)!=-1)       { count_157+=s.match(re157).length;s=s.replace(re157, re157_); }
	   if (s.search(re158)!=-1)       { count_158+=s.match(re158).length;s=s.replace(re158, re158_); }
	   if (s.search(re159)!=-1)       { count_159+=s.match(re159).length;s=s.replace(re159, re159_); }
	   if (s.search(re160)!=-1)       { count_160+=s.match(re160).length;s=s.replace(re160, re160_); }
	   if (s.search(re161)!=-1)       { count_161+=s.match(re161).length;s=s.replace(re161, re161_); }
// 	   if (s.search(re162)!=-1)       { count_162+=s.match(re162).length;s=s.replace(re162, re162_); }
	   if (s.search(re163)!=-1)       { count_163+=s.match(re163).length;s=s.replace(re163, re163_); }
	   if (s.search(re164)!=-1)       { count_164+=s.match(re164).length;s=s.replace(re164, re164_); }
	   if (s.search(re165)!=-1)       { count_165+=s.match(re165).length;s=s.replace(re165, re165_); }
...
//        if (count_130!=0)   {st2+='\n• Унификация закрывающих болдов и курсивов:	'+count_130;}
 ...
 if (count_142!=0)   {st2+='\n• Область для частного использования U+F04A:	'+count_142;}
//  if (count_143!=0)   {st2+='\n• Комбинируемое бреве+и на й:	'+count_143;}
 ...
 if (count_150!=0)   {st2+='\n• Минус на длинное тире:	'+count_150;}
 if (count_151!=0)   {st2+='\n• Идеограмма 一 на длинное тире:	'+count_151;}
 if (count_152!=0)   {st2+='\n• Полноширинная левая скобка на левую скобку:	'+count_152;}
 if (count_153!=0)   {st2+='\n• Полноширинная правая скобка на правую скобку:	'+count_153;}
//        if (count_154!=0)   {st2+='\n• Левая двойная угловая скобка на левую кавычку-елочку:	'+count_154;}
//        if (count_155!=0)   {st2+='\n• Правая двойная угловая скобка на правую кавычку-елочку:	'+count_155;}
 if (count_156!=0)   {st2+='\n• Метка справа-налево U+200F:	'+count_156;}
 if (count_157!=0)   {st2+='\n• Селекторы начертания (1-16):	'+count_157;}
 if (count_158!=0)   {st2+='\n• Стрелка влево с крючком U+21A9:	'+count_158;}
 if (count_159!=0)   {st2+='\n• Кавычки в названии изд-ва:	'+count_159;}
 if (count_160!=0)   {st2+='\n• Символ U+E098 после гласной на гласную с ударением:	'+count_160;}
 if (count_161!=0)   {st2+='\n• Цифра 2 внутри слова после гласной на гласную с ударением:	'+count_161;}
//  if (count_162!=0)   {st2+='\n• Ударения-апострофы после строчной гласной на гласную с ударением:	'+count_162;}
 if (count_163!=0)   {st2+='\n• Полноширинный дефис-минус на длинное тире:	'+count_163;}
 if (count_164!=0)   {st2+='\n• Область для частного использования U+F06E:	'+count_164;}
 if (count_165!=0)   {st2+='\n• Вопросительный восклицательный знаки U+2028:	'+count_165;}
 ...
 MsgBox ('                  –= Jurgen Script =– \n'+
        ' «Генеральная уборка - ИНОСТРАНКА» v.'+VersionNumber+' Golma+TaKir Edition 2026       \n\n'+
