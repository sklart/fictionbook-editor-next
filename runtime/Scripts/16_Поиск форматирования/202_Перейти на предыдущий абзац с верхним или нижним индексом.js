// Скрипт "Перейти на предыдущий абзац с верхним или нижним индексом" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для поиска и выделения абзацев, содержащих форматирование
// верхним или нижним индексом вверх по тексту fb2 документа.
// В настройках можно задать либо установку курсора, либо выделение всего найденного абзаца.

// На основе скриптов
// Перейти на предыдущий абзац с верхним индексом, Перейти на предыдущий абзац с нижним индексом
...
function Run() {
    ...
    var scriptName = "Перейти на предыдущий абзац с верхним или нижним индексом";
    var version = "1.0";
    ...
    function scrollIfItNeeds() {
        ...
        if (selection) {
            ...
            // Проверяем, находится ли выделение менее чем в 20 пикселях от верхнего края окна
            if (rect.top < 20) {
                // Прокручиваем документ на 50 пикселей вверх
    ...
    function mySearch() {
        ...
        if (paragraphIndent) {
            ...
            if (el == fbw_body) {
                if (confirm("Достигнуто " + (scriptDirection == "forward" ? "конец" : "начало") + " документа.\nПродолжить поиск с " + (scriptDirection == "forward" ? "начала" : "конца") + " документа?")) {
                    el = fbw_body;
                } else {
                    return;
                }
        ...
        // Теперь ищем новый индекс (SUP или SUB) в обратном направлении
        while (true) {
            el = getNextNodeInScriptDirection(el);
            if (el == placeWhereToStop) {
                alert("Ничего не найдено, поиск завершен.");
            ...
            if (isItTagWeLookingFor(el) && nonEmptyTag(el)) {
                ...
                if (actionOnFound == 0) {
                    ...
                    if (scriptDirection == "forward") {
                        ...
                        if (range1.parentElement() !== el && range1.move("character", -1) == 1) range1.move("character", 1);
                        // Специальная коррекция для обратного направления
    ...
    // Для обратного поиска начинаем с маркера начала (beginMarkerEl)
    // а останавливаемся на маркере конца (endMarkerEl)
