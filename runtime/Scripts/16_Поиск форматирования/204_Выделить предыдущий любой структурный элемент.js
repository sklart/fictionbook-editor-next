// Скрипт "Выделить предыдущий любой структурный элемент" для редактора FBE
// version 1.2
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для поиска и выделения уже размеченных элементов в fb2 документах.
// Поиск ведется от текущего положения курсора вверх по тексту до конца документа.
...
// Выделить предыдущий ... элемент (заголовок, цитату, стих), уважаемого тов. Sclex.

// version 1.2, 06.03.2026
//======================================

function Run() {
    // Название и версия для сообщений
    var scriptName = "Выделить предыдущий любой структурный элемент";
    var version = "1.2";
    ...
    // === Функции для навигации ВВЕРХ (из примера) ===
    ...
    // Функция для получения предыдущего любого элемента (не только P)
    ...
    window.external.BeginUndoUnit(document, "Поиск предыдущего элемента");
    try { window.external.SetStatusBarText("Ищем предыдущий структурный элемент..."); } catch(e) {}
    ...
    if (foundElement) {
        ...
        // ВСЕГДА показываем сообщение о достижении начала, независимо от режима!
        MsgBox(scriptName + "\nver. " + version + "\n\n" +
               "До начала документа не нашлось ни одного подходящего элемента.");
    }
