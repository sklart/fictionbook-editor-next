// Скрипт «Поиск возможных маркеров ТЕКСТОВ сносок» для редактора Fiction Book Editor (FBE).
// Основано на скрипте «Поиск по набору регэкспов»
// Версия 5.3

// Автор Sclex, набор RegExp-ов - TaKir, Sclex, 23.05.2026
...
// Скрипт предназначен для поиска ТОЛЬКО возможных типичных маркеров текстов сносок (МТС) в fb2 документах.
// Ищутся звёздочки или цифры в началах абзацев.
...
// -------------начало блока TaKir - регэкспы (23.05.2026)---------------

// Ищем звездочки в началах абзацев (возможные маркеры текстов сносок - МТС)

tagRegExp("^[\\*]{1,20}\\s*[A-Za-zА-яЁё0-9—…,;.»\"!?:%“™”©].+$","","Найдено: звездочки в начале строки с текстом (потенциальные маркеры текстов сносок)","-title -subtitle -stanza -poem -epigraph");

// Ищем возможные маркеры текстов сносок (МТС) в квадратных скобках в началах абзацев

tagRegExp("^(\\[(\\d{1,4})\\])","","Найдено: цифры в квадратных скобках в начале строки)","-title -subtitle -stanza -poem -epigraph");


// Ищем "списки" в началах абзацев (как частный случай маркеров текстов сносок - МТС)

tagRegExp("^([1-9]\\d{0,3})[\\)\\.]?\\s+(.+)$","","Найдено: Нумерованный список - цифры со скобками/точками/пробелом в начале строки)","-title -subtitle -stanza -poem -epigraph");

// -------------конец блока TaKir - регэкспы (23.05.2026):---------------
 ...
 function searchNext() {
   ...
   // ++++++++++++++++++++++++++++++++++++++++++++++++++++++
   MsgBox("От позиции курсора до конца документа МТС не найдено.");
 }
