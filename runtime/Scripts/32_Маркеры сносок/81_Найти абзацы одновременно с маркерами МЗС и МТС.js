// Скрипт «Найти абзацы одновременно с маркерами МЗС и МТС» для редактора Fiction Book Editor (FBE).
// Основано на скрипте «Поиск по набору регэкспов»
// Версия 5.3

// Автор Sclex, набор RegExp-ов - TaKir, Sclex, 11.08.2026
...
// 12 ноября 2025 переработаны почти все поисковые строки (TaKir)

// Скрипт предназначен для поиска абзацев, содержащих одновременно маркеры знаков сносок (МЗС)
// и маркеров текстов сносок (МТС) в fb2 документах.
// Скрипт ищет абзацы, в которых есть какие-либо типичные маркеры МЗС + МТС,
// например цифры верхним индексом, цифры в квадратных или фигурных скобках, звёздочки, решётки.
// МЗС маркер всегда расположен где-то внутри или в конце абзаца (не в начале).
// МТС маркер всегда расположен в начале абзаца.
// Абзацы, содержащие одновременно оба или более таких маркеров, вызывают проблемы при расстановке сносок.
// Также ищутся абзацы с цифрой 0 в верхнем индексе — возможные проблемные маркеры сносок.


// Данный скрипт лучше запускать после выполнения скриптов «Генеральная уборка» и «Расстановка ёлочек и лапок».
// Тогда будет гораздо меньше ложных срабатываний.

// Обновленный набор регэкспов от 11.08.2026 (TaKir)
//======================================
...
// -------------начало блока TaKir - регэкспы (11.08.2026)---------------

// Ищем абзацы с 2 или более возможными маркерами сносок (первый — в начале) — это возможные проблемные маркеры сносок:
// Маркер знака сноски (МЗС) - в начале абзаца и маркер текста сноски (МТС) - в любом другом месте этого же абзаца.


tagRegExp("^<пустота-теги-или-ничего><SUP>\\d+</SUP>.*<SUP>\\d+</SUP>.*$", "", "Найдено: абзац с 2+ маркерами-верхними индексами (первый в начале) — возможные проблемные маркеры сносок");

addRegExp("^\\[(\\d{1,4})\\].*\\[(\\d{1,4})\\].*$","","Найдено: абзац с 2+ маркерами-цифрами в квадратных скобках (первый — в начале) — возможные проблемные маркеры сносок");

addRegExp("^.*?\\{(\\d{1,4})\\}.*\\{(\\d{1,4})\\}.*$","","Найдено: абзац с 2+ маркерами-цифрами в фигурных скобках (первый — в начале) — возможные проблемные маркеры сносок");

addRegExp("^\\*.*[^\\s\\*].*\\*.*$","","Найдено: абзац с 2+ маркерами-звёздочками (первый — в начале) — возможные проблемные маркеры сносок");

addRegExp("^#.*[^\\s#].*#.*$","","Найдено: абзац с 2+ маркерами-решётками (первый — в начале) — возможные проблемные маркеры сносок");

// tagRegExp("^<пустота-теги-или-ничего>.*<SUP>0</SUP>.*$", "", "Найдено: абзац с цифрой 0 в верхнем индексе — возможные проблемные маркеры сносок");

tagRegExp(".*<SUP>(?:<[^>]+>)*0(?:<[^>]+>)*</SUP>.*", "", "Найдено: абзац с 0 в верхнем индексе  — возможный знак градуса Цельсия");


// -------------конец блока TaKir - регэкспы (11.08.2026):---------------
 ...
 function searchNext() {
   ...
   // ++++++++++++++++++++++++++++++++++++++++++++++++++++++
   MsgBox("От позиции курсора до конца документа проблемных абзацев с МЗС + МТС не найдено.");
 }
