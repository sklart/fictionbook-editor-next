// Скрипт «Расширенный поиск возможных маркеров сносок» для редактора Fiction Book Editor (FBE).
// Основано на скрипте «Поиск по набору регэкспов»
// Версия 5.3

// Автор Sclex, набор RegExp-ов - TaKir, Sclex, 07.02.2026
...
// Скрипт предназначен для поиска возможных типичных маркеров знаков сносок (МЗС)
...
// Обновленный набор регэкспов от 07.02.2026 (TaKir)
//======================================
...
// -------------начало блока TaKir - регэкспы (07.02.2026)---------------

// Ищем звездочки рядом с текстом (возможные маркеры знаков и текстов сносок - МЗС и МТС)

  tagRegExp("([A-Za-zА-яЁё0-9…,;.»\"!?:%“™”©\\)])(</?[^>A/][^>]*>)*[\*]","","Найдено: звездочки после букв, цифр и знаков препинания");
  tagRegExp("(?<![\\*])[\\*](?![\\*\\s])(</?[^>A/][^>]*>)*(?=([\\x20\\xA0])?[A-Za-zА-яЁё])","","Найдено: звездочки перед буквами, кроме подзаголовков");
  addRegExp("^[\\*]{1,20}\\s*[A-Za-zА-яЁё0-9—…,;.»\"!?:%“™”©].+$","","Найдено: звездочки в начале строки с текстом (потенциальные маркеры текстов сносок)");


// Ищем "прилипшие" к тексту цифры и числа (возможные маркеры знаков сносок - МЗС)

  addRegExp("(?<![.,]\\d)([A-Za-zА-яЁё][%…,;.»\"!?:%“™”©]*)([1-9]\\d{0,3})(?![.,]?\\d)","","Найдено: целые числа (1-4 цифры), прилипшие к словам");
  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©])\\((\\d{1,4})\\)","","Найдено: цифры в круглых скобках, прилипшие к словам");
  addRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©]){(\\d{1,4})}","","Найдено: целые числа в фигурных скобках, прилипшие к словам");
  tagRegExp("([A-Za-zА-яЁё…,;.»\"!?:%“™”©])\\[(\\d{1,4})\\](?![^<]*</a>)","","Найдено: цифры в квадратных скобках, прилипшие к словам");

  tagRegExp("[\]](</?[^>A/][^>]*>)*(?=([\\x20\\xA0])?[A-Za-zА-яЁё])","","Найдено: скобки перед буквами");
  
  addRegExp("(\\d\\s*[%!?:;»\"»]\\s*)([1-9]\\d*)", "", "Найдено: маркеры сносок (цифры) после цифр со знаками препинания");


// Ищем возможные маркеры текстов сносок (МТС) в квадратных скобках)

  addRegExp("^(\\[(\\d{1,4})\\])","","Найдено: цифры в квадратных скобках в начале строки");


// Ищем "списки" (как частный случай маркеров текстов сносок - МТС)

  addRegExp("^([1-9]\\d{0,3})[\\)\\.]?\\s+(.+)$","","Найдено: Нумерованный список - цифры со скобками/точками/пробелом в начале строки");


// Ищем верхние нижние индексы в квадратных скобках

tagRegExp("<SUP>\\[(\\d+)\\]</SUP>(?!<A class=note)", "", "Найдено: целые числа в квадратных скобках в верхнем индексе (кроме сносок)");

tagRegExp("<SUB>\\[(\\d+)\\]</SUB>(?!<A class=note)", "", "Найдено: целые числа в квадратных скобках в нижнем индексе (кроме сносок)");


// !!!!!!!!!!!!!!!!!!tagRegExp("(?<!<A class=note href=\"[^\"]*\">)(?<!<SUP>)\\[(\\d+)\\](?!</A>)(?!</SUP>)(?!</SUB>)", "", "Найдено: целые числа в квадратных скобках ТОЛЬКО в обычном тексте (кроме сносок)");


// -------------конец блока TaKir - регэкспы (07.02.2026):---------------
