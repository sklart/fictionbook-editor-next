// Скрипт «Поиск СМС-переписки» для редактора Fiction Book Editor (FBE).
// Основано на скрипте «Поиск по набору регэкспов», версия 5.3

// Автор Sclex, набор RegExp-ов - TaKir, Sclex, 25.05.2026
...
// Скрипт предназначен для поиска СМС-переписки в fb2 документах.
// От начала абзаца ищется текст до 15 символов перед двоеточием, как признак автора СМС-сообщения, и далее любой текст.
// Заголовки, подзаголовки, стихи, цитаты, эпиграфы исключены из поиска.
// Можно включить их в поиск, удалив из перечня упоминание нужных тэгов:
// -title -subtitle -stanza -poem -epigraph -cite
// См. ниже в разделе: начало блока TaKir - регэкспы...
...
// Обновленный набор регэкспов от 10.07.2026 (TaKir)
//======================================
...
// -------------начало блока TaKir - регэкспы (10.07.2026)---------------


// Ищем возможные признаки СМС-переписки в началах абзацев
// Текст до 15 символов перед двоеточием, как признак автора СМС-сообщения
// Абзацы, начинающиеся со знака @, как признак автора СМС-сообщения

// Вариант 1: Поиск по чистому тексту (без учёта тегов)
addRegExp("^([A-Za-zА-Яа-яЁё0-9_]{1,15}):\\s+.+$","","Найдено: возможная СМС переписка (чистый текст)","-title -subtitle -stanza -poem -epigraph -cite");

// Вариант 2: Поиск с учётом тегов (если возможный никнейм выделен жирностью/курсивом)
tagRegExp("<[^>]+>([A-Za-zА-Яа-яЁё0-9_]{1,15})</[^>]+>:\\s+.+$","","Найдено: возможная СМС переписка (с тегами)","-title -subtitle -stanza -poem -epigraph -cite");

tagRegExp("^<пустота-теги-или-ничего>@.+$","","Найдено: абзац, начинающийся со знака @","-title -subtitle -stanza -poem -epigraph -cite");


// -------------конец блока TaKir - регэкспы (10.07.2026):---------------
 ...
 function searchNext() {
   ...
   // ++++++++++++++++++++++++++++++++++++++++++++++++++++++
   MsgBox("От позиции курсора до конца документа СМС-переписки не найдено.");
 }
