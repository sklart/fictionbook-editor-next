// Скрипт "Перейти на следующую НЕ-пустую блочную иллюстрацию" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для поиска и выделения следующей "НЕ-пустой" блочной (отдельностоящей) иллюстрации
// (со ссылкой на прикреплённый бинарник) в fb2 документе.
// Пустые иллюстрации (пустышки без ссылки на бинарник) пропускаются.
...
function Run() {
    ...
    var scriptName = "Перейти на следующую НЕ-пустую блочную иллюстрацию";
    var version = "1.0";
    ...
    try { window.external.SetStatusBarText("Ищем следующую НЕ-пустую иллюстрацию..."); } catch(e) {}
    ...
    // === АЛГОРИТМ ПОИСКА НЕ-ПУСТЫХ ИЛЛЮСТРАЦИЙ ===
    ...
    // Поиск следующей НЕ-пустой иллюстрации
    ...
    if (foundImage) {
        if (selectImage(foundImage)) {
            if (showMessages) {
                MsgBox(scriptName + "\nver. " + version + "\n\n" +
                       "Найдена и выделена НЕ-пустая блочная иллюстрация\n\n" +
        ...
        MsgBox(scriptName + "\nver. " + version + "\n\n" +
               "До конца документа не нашлось ни одной НЕ-пустой блочной иллюстрации.");
    }
