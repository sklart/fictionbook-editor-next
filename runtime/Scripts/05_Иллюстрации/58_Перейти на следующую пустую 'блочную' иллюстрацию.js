// Скрипт "Перейти на следующую пустую блочную иллюстрацию" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для поиска и выделения следующей "пустой" блочной (отдельностоящей) иллюстрации
// (без ссылки на прикреплённый бинарник) в fb2 документе.
// Поиск ведётся от текущего положения курсора вниз по тексту до конца документа.
// Обычные иллюстрации (со ссылкой на прикреплённый бинарник) пропускаются.
// Найденная пустая иллюстрация выделяется целиком и прокручивается в центр окна для удобства.
...
// На основе скрипта "Перейти на следующую блочную иллюстрацию" ver. 1.2
...
function Run() {
    ...
    var scriptName = "Перейти на следующую пустую блочную иллюстрацию";
    var version = "1.0";
    ...
    // Функция проверки - является ли блочная иллюстрация пустышкой
    // (у DIV.image href содержит "undefined" И у вложенного IMG src содержит "undefined")
    function isEmptyImage(imageDiv) {
    ...
    // Функция для получения следующего любого элемента
    ...
    try { window.external.SetStatusBarText("Ищем следующую пустую иллюстрацию..."); } catch(e) {}
    ...
    // === АЛГОРИТМ ПОИСКА ПУСТЫХ ИЛЛЮСТРАЦИЙ ===
    ...
    if (foundImage) {
        if (selectImage(foundImage)) {
            if (showMessages) {
                MsgBox(scriptName + "\nver. " + version + "\n\n" +
                       "Найдена и выделена пустая блочная иллюстрация\n\n" +
        ...
        // ВСЕГДА показываем сообщение о достижении конца
        MsgBox(scriptName + "\nver. " + version + "\n\n" +
               "До конца документа не нашлось ни одной пустой блочной иллюстрации.");
    }
