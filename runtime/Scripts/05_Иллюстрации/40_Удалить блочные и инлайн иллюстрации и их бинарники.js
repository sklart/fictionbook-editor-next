// Скрипт "Удалить все блочные и инлайн иллюстрации и их бинарники" для редактора FBE
// version 1.2
// Идея - TaKir
...
// Скрипт предназначен для удаления блочных иллюстраций (DIV с классом image)
// и инлайн иллюстраций (SPAN с классом image) и их бинарных файлов в fb2 документе.
...
// version 1.2, 08.02.2026
//======================================

function Run() {
    // Название и версия для сообщений
    var scriptName = "Удалить все блочные и инлайн иллюстрации и их бинарники";
    var version = "1.2";
    ...
    var processInlineMarkers = 0;        // 0 - нет, 1 - да
    ...
    var removeEmptyLines = 1;      // 1 - удалять пустые строки вокруг блочных иллюстраций, 0 - нет
    ...
    try {
        ...
        analyzeDocument(stats, binaryIdsToDelete, coverBinaryIds);
        ...
        if (!hasImagesToRemove) {
            ...
            if (!removeBlockImages && !removeInlineImages) {
                message += "В настройках отключено удаление и блочных, и инлайн иллюстраций.\n" +
                          "Измените настройки скрипта.";
            } else if (removeBlockImages && removeInlineImages) {
                message += "Не найдено ни блочных, ни инлайн иллюстраций для удаления.";
            } else if (removeBlockImages) {
                message += "Не найдено блочных иллюстраций для удаления.";
            } else {
                message += "Не найдено инлайн иллюстраций для удаления.";
            }
        ...
        if (showStatistics) {
            ...
            var foundBlock = removeBlockImages ? stats.blockImages : 0;
            var foundInline = removeInlineImages ? stats.inlineImages : 0;
            ...
            analysisMessage += "\n\nВыполнить удаление иллюстраций и их бинарников?";
        ...
        if (stats.blockBinaries > 0 || stats.inlineBinaries > 0) {
            firstRemoving = removeCorrespondingBinaries(binaryIdsToDelete, coverBinaryIds, stats, firstRemoving, undoUnitName);
        }
        ...
        if (showStatistics) {
            ...
            var totalRemoved = stats.blockRemoved + stats.inlineRemoved;
            
            if (totalRemoved > 0) {
                resultMessage += "✓ Удалено иллюстраций: " + totalRemoved + "\n";
            }
            ...
            if (stats.coverBinaries > 0) {
                resultMessage += "\n• Обложки сохранены: " + stats.coverBinaries + " шт.\n";
            }
...
function analyzeDocument(stats, binaryIdsToDelete, coverBinaryIds) {
    // Считаем бинарники
...
function removeCorrespondingBinaries(binaryIdsToDelete, coverBinaryIds, stats, firstRemoving, undoUnitName) {
    var binobj = document.all.binobj;
