// Скрипт "Перейти на предыдущую НЕ-пустую блочную иллюстрацию" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для поиска и выделения предыдущей "НЕ-пустой" блочной (отдельностоящей) иллюстрации
...
function Run() {
    ...
    var scriptName = "Перейти на предыдущую НЕ-пустую блочную иллюстрацию";
    var version = "1.0";
    ...
    try { window.external.SetStatusBarText("Ищем предыдущую НЕ-пустую иллюстрацию..."); } catch(e) {}
    ...
    // === АЛГОРИТМ ПОИСКА НЕ-ПУСТЫХ ИЛЛЮСТРАЦИЙ ВВЕРХ ===
    ...
    // Поиск предыдущей НЕ-пустой иллюстрации
    ...
    if (foundImage) {
        ...
        MsgBox(scriptName + "\nver. " + version + "\n\n" +
               "До начала документа не нашлось ни одной НЕ-пустой блочной иллюстрации.");
    }
