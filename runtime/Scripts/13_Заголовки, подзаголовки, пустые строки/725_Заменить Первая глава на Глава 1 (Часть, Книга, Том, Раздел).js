// Скрипт "Заменить Первая глава на Глава 1 (Часть, Книга, Том, Раздел)" для редактора FBE
// version 1.3
// Идея - TaKir
...
// Тридцать пятая глава ==> Глава 35, Четвёртый том ==> Том 4
...
// version 1.3, 31.03.2026
//======================================

function Run() {
    // Название и версия для сообщений
    var scriptName = "Заменить Первая глава на Глава 1 (Часть, Книга, Том, Раздел)";
    var version = "1.3";
    ...
    function getFemaleNumber(text) {
        ...
        if (hundredsFemaleToNum[lowerText]) {
            return hundredsFemaleToNum[lowerText];
        }
        
        // Проверяем 10-19
        if (teensFemaleToNum[lowerText]) {
            return teensFemaleToNum[lowerText];
        }
        
        // Проверяем круглые десятки
        if (tensFemaleToNum[lowerText]) {
            return tensFemaleToNum[lowerText];
        }
        
        // Проверяем единицы
        if (unitsFemaleToNum[lowerText]) {
            return unitsFemaleToNum[lowerText];
        }
    ...
    function getMaleNumber(text) {
        ...
        // Проверяем круглые сотни
        if (hundredsMaleToNum[lowerText]) {
            return hundredsMaleToNum[lowerText];
        }
        
        // Проверяем 10-19
        if (teensMaleToNum[lowerText]) {
            return teensMaleToNum[lowerText];
        }
        
        // Проверяем круглые десятки
        if (tensMaleToNum[lowerText]) {
            return tensMaleToNum[lowerText];
        }
        
        // Проверяем единицы
        if (unitsMaleToNum[lowerText]) {
            return unitsMaleToNum[lowerText];
        }
    ...
    function replaceChapterBack(match, numberText, type) {
        ...
        // Всегда делаем тип с заглавной буквы
        var capitalizedType = typeLower.charAt(0).toUpperCase() + typeLower.slice(1);
    ...
    function processTextNode(node, isInTitle) {
        ...
        // Ищем: текстовое числительное (одно или два слова) + пробел + (глава|часть|книга|том|раздел)
        var regex = /([а-яё]+(?:\s+[а-яё]+)?)\s+(глава|часть|книга|том|раздел)/gi;
        
        var newText = text.replace(regex, function(match, numberText, type) {
