// Скрипт "Заменить Глава 1 на Глава первая (Часть, Книга, Том, Раздел)" для редактора FBE
// version 1.4
// Идея - TaKir
...
// Глава 35 ==> Глава тридцать пятая, Том 4 ==> Том четвертый
...
// version 1.4, 30.03.2026
//======================================

function Run() {
    // Название и версия для сообщений
    var scriptName = "Заменить Глава 1 на Глава первая (Часть, Книга, Том, Раздел)";
    var version = "1.4";
    ...
    var processOnlyTitles = 0; // По умолчанию - 1 (только заголовки)
    ...
    function getFemaleText(numStr) {
        ...
        if (num >= 101 && num <= 999) {
            ...
            if (remainder == 0) {
                // Круглые сотни уже обработаны выше, но на всякий случай
    ...
    function replaceChapter(match, type, num) {
        ...
        if (type.charAt(0) == type.charAt(0).toUpperCase()) {
            return type + ' ' + numberText;
        } else {
            return typeLower + ' ' + numberText;
        }
    }
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (child.nodeType == 3) { // текстовый узел
                ...
                if (isFormattingTag(child)) {
                    ...
                    // Любой другой тег (не форматирования) — считаем его значимым и ищем внутри
                    var innerText = getFirstSignificantText(child);
                    if (innerText && innerText.length > 0) {
                        return innerText;
                    }
    ...
    function isParagraphStartWithKeyword(paragraphElement) {
        ...
        // Ключевые слова: Глава, Часть, Книга, Том, Раздел (с учетом регистра)
