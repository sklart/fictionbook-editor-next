// Скрипт "Разметить подзаголовки из курсивных абзацев без точки в конце" для редактора FBE
// version 1.0
// Идея - TaKir
// Реализация - DeepSeek, TaKir

// Скрипт предназначен для автоматической разметки подзаголовков в fb2 документах.
// Находит абзацы, целиком выделенные курсивом (<P><EM>...</EM></P>),
...
// опционально удаляя внешние теги курсива.
...
var scriptName = "Разметить подзаголовки из курсивных абзацев без точки в конце";
var version = "1.0";
...
var allowedEndChars = "!?…:;»\"]";
    ...
    // Размечать ли курсивные абзацы, похожие на диалоги (начинающиеся с тире/дефиса)
var processDialogs = 0;     // 0 - нет, 1 - да

    // Удалять ли тэги курсива после преобразования (только внешний EM)
var removeEm = 1;     // 0 - нет, 1 - да (по умолчанию удаляем)
...
function isLineEmpty(ptr) {
    ...
    // Функция проверки, является ли абзац целиком курсивным
function isEntirelyItalic(elem) {
    if (!elem || isLineEmpty(elem)) return false;
    
    var myHtml = elem.innerHTML;
    var reEm = /<EM>(((?!<\/?EM>).)*?)<\/EM>/ig;
    var reTags = /<\/?[^>]*>/ig;
    var reSpaces = new RegExp("^[\\s" + nbspChar + "&nbsp;]*$", "i");
    
        // Удаляем все вхождения <EM>...</EM>
    var searchResult = reEm.test(myHtml);
    while (searchResult) {
        myHtml = myHtml.replace(reEm, "");
        searchResult = reEm.test(myHtml);
    }
    
        // Удаляем все оставшиеся теги
    myHtml = myHtml.replace(reTags, "");
    
        // Если после удаления курсивных тегов остался только текст с пробелами - значит абзац был целиком курсивным
...
function getBodyType(bodyElement) {
    ...
    // Функция удаления внешних тегов EM из абзаца (с сохранением внутреннего форматирования)
function removeOuterEmTags(ptr) {
    if (!ptr) return;
    
    var html = ptr.innerHTML;
        // Удаляем только внешние открывающие и закрывающие теги EM
...
var potentialSubtitles = [];
    ...
    // Статистика
var stats = {
    totalItalic: 0,               // всего целиком курсивных абзацев
...
for (var b = 0; b < bodies.length; b++) {
    ...
    for (var i = 0; i < paragraphs.length; i++) {
        ...
        if (isLineEmpty(p)) continue;
            ...
            // Проверка: абзац должен быть целиком курсивным
        if (!isEntirelyItalic(p)) continue;
        
            // Нашли целиком курсивный абзац
        stats.totalItalic++;
...
if (showStatistics == 1) {
    ...
    if (stats.totalFound == 0) {
        ...
        msg += "• Удалять внешний <em>: " + (removeEm ? "да" : "нет") + "\n\n";
        ...
        msg += "✓ Всего целиком курсивных абзацев: " + stats.totalItalic + "\n";
        ...
        msg += "Преобразовать найденные курсивные абзацы в подзаголовки?";
...
if (userConfirmed && potentialSubtitles.length > 0) {
    ...
    for (var i = 0; i < potentialSubtitles.length; i++) {
        ...
        p.className = "subtitle";
            ...
            // Удаляем внешние теги em, если нужно
        if (removeEm) {
            removeOuterEmTags(p);
        }
