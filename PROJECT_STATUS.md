...
Текущая версия исходников: `3.0.7` (предварительный выпуск `3.0.7-rc.1`).

# Текущий статус

Исторический снимок состояния: 17 июля 2026 года. Он не является актуальным
статусом: текущие изменения и проверки фиксируются в `CHANGELOG.md`,
`docs/todo.md` и `docs/architecture/verification-status.md`.

## Завершено

- Оптимизирован CI/release-контур двух профилей: общие Win32-бинарники,
  property handler и MUI собираются один раз на Modern-этапе, тогда как Win7
  повторно собирает только Scintilla/Lexilla. Portable-упаковка берёт эти DLL
  из явно указанного целевого каталога, а release-проверка сравнивает хэши
  общих файлов двух архивов и подтверждает отдельный вариант Scintilla.

- Release-контур поддерживает предварительные теги вида `vX.Y.Z-rc.N`: они
  публикуются GitHub как предварительные выпуски, используют версию исходников `X.Y.Z` и
  не изменяют стабильный `update.xml`. Проверка архивов учитывает, что три
  Batch-конвертера намеренно различаются между Modern и Win7 пакетами.

- Пункт «Сервис → Диагностическая трассировка…» синхронизирован с русским и
  украинским сгенерированным MENU-ресурсом; он доступен в локализованных сборках,
  а не только при встроенном английском меню.

- Исправлен ложный блокирующий gate GitHub Actions: аудит кириллицы в C/C++
  строковых литералах остался видимой частью release-проверки, но не
  останавливает выпуск из-за существующего исторического набора строк.
  Строгий режим применяется только в изолированных regression-fixture до
  поэтапного выноса оставшихся строк в локализации.

- В Win7 portable-пакете пакетные конвертеры `ExportDOCXBatch`,
  `ExportEPUBBatch` и `ImportEPUBBatch` дополнительно пересобираются с
  API-уровнем Windows 7. Это исключает импорт `CreateFile2` EPUB-конвертером;
  запуск Batch EXE двойным щелчком показывает подсказку вместо мгновенно
  исчезающего консольного окна.

- Выполнен первый этап issue #1: на вкладке «Настройки FBE Next» можно выбрать классическую, контрастную или тёмную XML-палитру Scintilla. Выбор сохраняется, классическая палитра оставлена значением по умолчанию; элементы подключены к JSON-локализации и русской/украинской resource DLL. Исправлены конфликт идентификаторов новых контролов и две причины COM-ошибки `0x80070057` при переходе в режим «Код»: неверный вариант Active Scripting и неподдерживаемый MSXML6 `put_text` элемента `<binary>`. Настройка отдельных цветов, import/export палитр и FB2-семантические стили остаются следующими этапами.

- Исправлена настройка отображения неразрывного пробела: символы в списке заданы Unicode-кодами, а изменение заменяет фактический символ U+00A0 в DOM уже открытой книги.

- Исправлена загрузка пользовательских скриптов portable-версии из путей с
  кириллицей: путь больше не собирается в фиксированном буфере `MAX_PATH`, а
  папка `Scripts` сохраняется в исходном Unicode-регистре. Диагностический
  журнал фиксирует используемый каталог и число найденных скриптов.

- Диагностика работы FBE Next записывается в журнал
  `%LOCALAPPDATA%\FBE Next\fbe-trace.log`. Он включается переменной
  `FBE_NEXT_TRACE` либо через «Сервис → Диагностическая трассировка…» для
  следующего запуска; отдельные устаревшие переменные startup/selection удалены.
  В журнале различаются запуск, команды, перенос выделения, работа с документом,
  восстановление, COM и ошибки. В диагностическом режиме в заголовке окна есть
  пометка «Диагностика», а при запуске предлагается отключить режим для
...
- В рамках #24 base64-содержимое `binary` при сохранении FB2 переводится в
  ...
  автоматически создаваемом `binary id` больше не отбрасывается. Для
  совместимости с MSXML6 текст `<binary>` заменяется дочерним DOM-узлом, а не
  свойством `put_text` элемента; эта оптимизация выполняется только при
  сохранении, а не при открытии режима «Код». Нужна
...
- Подготовлен диагностический журнал для исследования переноса выделения между «Телом», «Описанием» и «Кодом»: при `FBE_NEXT_TRACE=1` создаётся `%LOCALAPPDATA%\FBE Next\fbe-trace.log` с категориями `startup`, `command`, `selection`, `document`, `recovery`, `script`, `com` и `error`. В шапке указаны версия FBE и архитектура ОС; журнал связывает команды меню, панели инструментов и горячих клавиш (когда источник различим в `WM_COMMAND`) с переходами, открытием, сохранением, `.bak`, проверкой книги, автосохранением, запуском пользовательского скрипта, созданием XML DOM и показываемыми HRESULT/COM-сбоями; ожидаются тестовая книга и журнал от сообщества.
...
- `update.xml` содержит сведения о стабильном релизе 3.0.6. Предварительные
  выпуски 3.0.7-rc.N его не изменяют; обновлять манифест до 3.0.7 следует
  только вместе с готовыми стабильными setup-артефактами и их SHA-256.
...
- Продукт: FictionBook Editor Next 3.0.7 (предварительный выпуск).
