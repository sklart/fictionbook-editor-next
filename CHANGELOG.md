...
- Исправлена вставка блочной иллюстрации в начале или внутри форматированного
  текста: `DIV.image` теперь структурно размещается между абзацами, поэтому
  форматирование и порядок текста сохраняются после Save → Reopen.
...
- Пользовательские панели скриптов завершены поверх `Toolbars.xml v2`: в
  ...
  переставлять и скрывать панели; основная `scripts-main` защищена. Изменения
  атомарно сохраняются как в portable, так и в installed mode, а при ошибке
  сохранения или создания runtime-контролов откатываются. Dynamic rebar bands
  корректно удаляются вместе с HWND, а временно отсутствующий скрипт сохраняет
  UID и исходную позицию среди кнопок и разделителей.
...
- Собственные C++-проекты теперь явно используют централизованный baseline
  ...
  C++20 override и исключение C/vendored/generated кода.

- В «Настройках FBE Next» появился канал обновлений: стабильные версии по умолчанию либо Beta/RC. Проверка использует SemVer, поэтому RC корректно сменяется финальным выпуском без downgrade. Окно «О программе» показывает номер найденного выпуска и открывает его конкретную страницу «Что нового…» на GitHub. Stable и prerelease feeds публикуются раздельно; GitHub Release description остаётся единственным актуальным текстом примечаний и не перезаписывается повторным CI.
...
- Обновлены редакторные зависимости: Scintilla до 5.6.6 и Lexilla до 5.5.3. В режиме «Код» ширина поля номеров строк теперь проверяется только при изменении количества строк, а подсветка парных XML-тегов не пересчитывается при простой прокрутке.
