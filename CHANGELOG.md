...
- FBE подключён как шестой runtime JSON-потребитель: единая функция `FbeLoadString` и `U::MessageBox` теперь сначала ищут строки в `Lang/<локаль>/fbe.json`, а затем безопасно откатываются к встроенным ресурсам. В текущий runtime-слой FBE входят 128 строк обновлений, сообщений, статусов, настроек, hotkey-названий и diagnostics; контракт сторожит `test-fbe-runtime-lang-overlay.ps1`.
