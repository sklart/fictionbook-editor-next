...
- Подготовлен следующий безопасный шаг к переключению языка: при смене языка в настройках FBE новая locale сразу публикуется в общий runtime-контракт, cache `fbe.json` сбрасывается, а `RefreshLocalizedMainFrameUi()` перезагружает главное меню, динамические пункты Import/Export/Script/MRU, hotkey-подписи, статические подписи toolbar-полей, тексты кнопок command/script toolbar, runtime-tooltip main toolbar без устаревшего WTL `LoadString` cache, status-pane INS/OVR и заголовок и внутреннее меню панели структуры документа. До полного live-refresh всех старых Win32-окон FBE снова честно предлагает перезапуск после смены языка; окна поиска/замены и проверки орфографии закрываются перед применением настроек и при следующем открытии создаются уже на новом языке.
