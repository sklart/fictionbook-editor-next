...
- Исправлено отсутствие пункта «Диагностическая трассировка…» в меню «Сервис»
...
- Добавлен включаемый переменной окружения `FBE_NEXT_TRACE` или через «Сервис →
  Диагностическая трассировка…» журнал `%LOCALAPPDATA%\FBE Next\fbe-trace.log`:
  в каждой строке отмечена категория (`startup`, `command`, `selection`,
  `document`, `recovery`, `script`, `com` или `error`), а в шапке — версия FBE и
  архитектура ОС. Журнал фиксирует команды главного меню, панели инструментов и
  горячих клавиш (когда источник различим в `WM_COMMAND`), перенос выделения,
  открытие, сохранение, `.bak`, проверку книги, автосохранение, запуск
  пользовательского скрипта, создание XML DOM и показываемые COM/HRESULT-сбои.
  Пункт меню изменяет режим для следующего запуска и предупреждает о
  необходимости перезапуска; при работе с журналом в заголовке отображается
  «Диагностика», а на запуске предлагается отключить режим. Прежние отдельные
  переменные трассировки запуска и переноса выделения удалены.
- На вкладке «Настройки FBE Next» добавлен выбор цветовой схемы XML-подсветки: классическая (по умолчанию), контрастная и тёмная. Выбор сохраняется; подписи доступны во всех 12 runtime-локализациях и встроенных русской/украинской resource DLL.
