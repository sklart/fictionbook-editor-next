...
private:
    ...
    static VOID CALLBACK uistone_TimerProc (HWND /* unused: hwnd */, UINT /* unused: uMsg */, UINT_PTR idEvent, DWORD /* unused: dwTime */)
    {
        if (!g_timer_list)
            return ;
...
public:
    ...
    {
        m_msg_queue.emplace_back(nType, nTaskID) ;
        m_timer.Start() ;
    }
...
private:
    ...
    public:
        ...
        bool Write (const void* p, int nLen)
        {
            Lock() ;
            if (!p || nLen < 0 || m_current_length > INT_MAX - nLen)
            {
                UnLock() ;
                return false ;
            }

            const int requiredSize = m_current_length + nLen ;
            if (requiredSize > m_max_size)
            {
                int newSize = m_max_size ;
                if (newSize == 0)
                    newSize = GLOW_MEM_SIZE ;
                while (newSize < requiredSize)
                {
                    if (newSize > INT_MAX - GLOW_MEM_SIZE)
                    {
                ...
                BYTE* newBuffer = (BYTE*)realloc(m_pBuf, newSize) ;
                if (!newBuffer)
                {
                    UnLock() ;
                    return false ;
...
public:
          ...
          m_event_observer(pObserver)
    {
        ...
        if (task_info.m_proxy_ip.GetLength())
        {
            ...
            m_session = InternetOpen (static_cast<LPCTSTR>(task_info.m_user_agent), INTERNET_OPEN_TYPE_PROXY, static_cast<LPCTSTR>(s), NULL, 0) ;
        }
        else
        {
            m_session = InternetOpen (static_cast<LPCTSTR>(task_info.m_user_agent), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0) ;
        }
        m_request = NULL ;
        ...
        {
            DWORD timeout = 30000 ;
            InternetSetOption(m_session, INTERNET_OPTION_CONNECT_TIMEOUT,
                &timeout, sizeof(timeout)) ;
            InternetSetOption(m_session, INTERNET_OPTION_SEND_TIMEOUT,
                &timeout, sizeof(timeout)) ;
            InternetSetOption(m_session, INTERNET_OPTION_RECEIVE_TIMEOUT,
                &timeout, sizeof(timeout)) ;
        }

        // set proxy username and password
        if (m_session && task_info.m_proxy_username.GetLength())
        ...
        if (m_session && task_info.m_proxy_password.GetLength())
    ...
    */
    void PopReceived (BYTE*& pBuffer, int& nLength)
    {
        ...
        if (nLength)
        {
            pBuffer = new (std::nothrow) BYTE[nLength + 2] ; // easy to convert string
            if (!pBuffer)
            {
                nLength = 0 ;
                m_read_buf.UnLock() ;
...
private:
    ...
    static DWORD WINAPI http_read_proc (LPVOID lpParameter)
    {
        ...
        while (true)
        {
            ...
            if (dwReadResult && dw)
            {
                if (!p->m_read_buf.Write(buf, (int)dw))
                {
                    dwReadResult = FALSE ;
                    break ;
                }
...
protected:
    ...
    */
    virtual void OnAfterDownloadConnected (FCHttpDownload* /* unused: pTask */) {}
    /**
        Callback on after download finish \n
        after this callback, system will delete this download task.
    */
    virtual void OnAfterDownloadFinish (FCHttpDownload* /* unused: pTask */) {}
    //@}
