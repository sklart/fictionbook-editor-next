...
int	StartScript(CMainFrame* mainframe) {
  ...
  g_site = new (std::nothrow) ScriptSite(mainframe);
  if (g_site == NULL)
    return -1;

  g_CB = new (std::nothrow) ScriptCB();
  if (g_CB == NULL) {
    StopScript();
    return -1;
  }
...
HRESULT	ScriptLoad(const wchar_t *filename) {
  ...
  if (!GetFileSizeEx(hFile, &fileSize) || fileSize.QuadPart < 0 || fileSize.QuadPart > static_cast<LONGLONG>(ULONG_MAX) - 2) {
    DWORD error = GetLastError();
    CloseHandle(hFile);
    return HRESULT_FROM_WIN32(error ? error : ERROR_FILE_TOO_LARGE);
  }

  DWORD length = static_cast<DWORD>(fileSize.QuadPart);
  SetFilePointer(hFile, 0, NULL, FILE_BEGIN);

  BYTE *tmp = new (std::nothrow) BYTE[length + 2];
  if (tmp == NULL) {
    CloseHandle(hFile);
  ...
  DWORD readError = ok ? ERROR_SUCCESS : GetLastError();
  CloseHandle(hFile);

  if (!ok || nrd != length) {
    delete[] tmp;
    return HRESULT_FROM_WIN32(ok ? ERROR_HANDLE_EOF : readError);
  }
  ...
  IMultiLanguage2 *pimlang2 = NULL;
  hr = CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_ALL, IID_IMultiLanguage2, (void **)&pimlang2);
  if (FAILED(hr) || !pimlang2) {
    pF->Release();
    delete[] tmp;
