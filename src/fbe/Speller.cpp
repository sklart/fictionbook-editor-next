...
void CSpeller::Replace(int nIndex)
{
	...
	{ 
		...
		if (m_numAphChanged) replace.Replace(L"'", L"Т");
...
void CSpeller::Replace(CString word)
{
	if (m_selRange)
	{
		if (m_numAphChanged) word.Replace(L"'", L"Т");
...
SPELL_RESULT CSpeller::SpellCheck(CString word)
{
	...
	if (currDict)
	{
		...
		m_numAphChanged = checkWord.Replace(L"Т", L"'");
		// remove all soft hyphens
		checkWord.Replace(L"\u00AD", L"");
		// remove accent
		checkWord.Replace(L"\u0301", L"");
		// special case for Russian letter "Є"
		if (currDict == m_Dictionaries[LANG_RU].handle) checkWord.Replace(L"Є", L"е");
...
//
// ¬озвращает абзац, содержащий заданный элемент. ¬ыделение MSHTML может
// находитьс€ внутри EM, A или другого inline-элемента, но подчЄркивани€
// должны быть св€заны с устойчивым контейнером P.
//
...
//
// »щет следующий абзац через DOM-соседей, не стро€ коллекцию всех P
// документа. Ёто сохран€ет локальность правки в конце длинного раздела.
//
...
//
// ѕровер€ет абзац, содержащий выделение.
//
void CSpeller::CheckElement(MSHTML::IHTMLElementPtr elem, long uniqID)
{
	...
	if (!innerText.Trim().IsEmpty())
	{
		...
		// ƒобавление inline-тега мен€ет только этот абзац. ќчистка всех
		// подчЄркиваний делала локальную правку пропорциональной размеру документа.
		ClearMarks(uniqID);
...
void CSpeller::CheckCurrentPage()
{
	...
	// ѕровер€ем только видимые абзацы. ≈сли нижн€€ граница не определена,
	// берЄм небольшое число следующих абзацев. ќбход начинаетс€ с видимого P,
	...
	do
	{
		...
		switch (result)
		{
			case SPELL_CHANGEALL:
			{
				...
				if (m_numAphChanged) replaceStr.Replace(L"'", L"Т");
