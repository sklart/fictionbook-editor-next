...
CString ScriptToolbarCollection::NextDefaultName(const CString& format) const
{
	for(unsigned int number = 1; ; ++number)
	{
		CString candidate; candidate.Format(format, number);
		bool used = false;
		for(size_t index = 0; index < m_items.size(); ++index)
			if(m_items[index].id != L"scripts-main" && m_items[index].name.CompareNoCase(candidate) == 0) { used = true; break; }
