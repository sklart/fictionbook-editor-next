...
LRESULT CScriptToolbarManagerDlg::OnInitDialog(UINT, WPARAM, LPARAM, BOOL&) { m_list = GetDlgItem(IDC_SCRIPT_PANELS_LIST); m_lastCommitted=m_manager.Collection().Items(); Refresh(); return TRUE; }
...
bool CScriptToolbarManagerDlg::Commit() { if(!m_changed()) { m_manager.Collection().Items()=m_lastCommitted; ::MessageBox(m_hWnd,FbeLoadRuntimeStringByKey(L"fbe.script_toolbar_manager.save_failed",L"Unable to save script toolbar settings."),FbeLoadRuntimeStringByKey(L"fbe.script_toolbar_manager.caption",L"FictionBook Editor"),MB_OK|MB_ICONERROR); Refresh(); return false; } m_lastCommitted=m_manager.Collection().Items(); Refresh(); return true; }
