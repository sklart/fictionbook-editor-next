...
public:
	...
	BEGIN_MSG_MAP(CTreeWithToolBar)
		...
		COMMAND_ID_HANDLER(ID_DOCUMENT_TREE_MODE_STRUCTURE, OnShowDocumentStructure)
		COMMAND_ID_HANDLER(ID_DOCUMENT_TREE_MODE_SCRIPTS, OnShowScripts)
	...
	LRESULT OnToolTipText(int idCtrl, LPNMHDR pnmh, BOOL& /*bHandled*/)
	{
		...
		if((idCtrl != 0) && !(pDispInfo->uFlags & TTF_IDISHWND))
		{
			...
			szBuff[0] = 0;
			int nRet = FbeLoadString(ModuleHelper::GetResourceInstance(), idCtrl, szBuff, cchBuff);
