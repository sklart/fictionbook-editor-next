...
vector< pair<int, int> > XmlMatchedTagsHighlighter::getAttributesPos(int start, int end)
{
	...
	for (; i < bufLen ; i++)
	{
		...
		if (state == attr_valid)
		{
			attributes.emplace_back(start+startPos, start+i+oneMoreChar);
			state = attr_invalid;
		}
	}
	if (state == attr_value)
		attributes.emplace_back(start+startPos, start+i-1);
	...
	if (patt) 
	{
		...
		while (pos >=0 && pos < docLen)
		{
			...
			if (pos >=0 && pos < docLen)
			{
				...
				int tagLength = pos - tagPos;
				char *tmp = (char *)malloc(tagLength + 1);
				if (!tmp)
					break;
				...
				if (!isSingleTag)
				{
					...
					else tags.emplace_back(tagName,tagPos);
				}
			}
		}
	}
