...
LRESULT CSettingsEditorPage::OnBrowseBackground(WORD, WORD, HWND, BOOL&)
{
	...
	if(result.outcome == ModernFileDialog::Outcome::Accepted && !result.paths.empty()) { m_customBackgroundPath = result.paths.front().c_str(); m_backgroundImage.SetCurSel(static_cast<int>(m_builtInBackgrounds.size() + 1)); }
	return 0;
}
