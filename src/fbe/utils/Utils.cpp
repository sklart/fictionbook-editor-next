...
void SetCurrentDirectoryToFile(const CString& filename)
{
  CString directory(filename);
  const int bufferLength = directory.GetLength() + 1;
  TCHAR* path = directory.GetBuffer(bufferLength);
  const HRESULT hr = ::PathCchRemoveFileSpec(path, static_cast<size_t>(bufferLength));
  directory.ReleaseBuffer();

  if (SUCCEEDED(hr) && !directory.IsEmpty())
...
CString GetUserDataFile(const CString& filename, const CString& legacyDir)
{
	...
	const CString destination = GetSettingsDir() + filename;
	if (!ATLPath::FileExists(destination))
	{
		CString legacyFile;
...
CString GetModulePath(HMODULE module)
{
  ...
  {
    CString path;
    TCHAR* buffer=path.GetBuffer(capacity);
    const DWORD length=::GetModuleFileName(module,buffer,capacity);
    if (length==0)
    {
...
CString	GetProgDir() {
  CString exedir=GetModulePath(_Module.GetModuleInstance());
  int len=exedir.GetLength();
  int p=len;
...
cfexit:
	...
	if(fileSize.QuadPart < 0 || fileSize.QuadPart > ULONG_MAX)
	{
		hr = HRESULT_FROM_WIN32(ERROR_FILE_TOO_LARGE);
		goto cfexit;
	}

	ULONG fsz = static_cast<ULONG>(fileSize.QuadPart);
...
// DomPath
	...
	{
		// лепим путь к элементу начина€ от него и раскручива€ до корн€
		m_path.clear();
	...
	bool DomPath::CPFT(const wchar_t* xml, int pos, int* char_pos)
	{
		...
		*char_pos = virtual_pos;
		...
		/*while(curpos = wcsstr(curpos, L"<"))
		{
			...
			// провер€ем не коментарий ли это
			...
			// провер€ем нужет ли закрывающий тег
			...
			// эту ситуацию надо об€зательно обработать*/
	...
	int DomPath::GetNodeFromText(wchar_t* xml, int char_pos)
	{
		...
		// считаем позицию, выкидыва€ вложеные ноды
	...
	bool tGetFirstXMLNodeParams(const wchar_t* xml, wchar_t** open_tag_begin, wchar_t** open_tag_end, wchar_t** close_tag_begin, wchar_t** close_tag_end)
	{
		...
		// возвращаем затребованные значени€
		if(open_tag_begin)
		{
			*open_tag_begin = optb;
		}
	...
	/*MSHTML::IHTMLDOMNodePtr GetNodeFromPos(MSHTML::IHTMLElementPtr elem, int pos)
	{
		...
		int index = 0; // по€рдковый номер ноды в HTML элементе

		// по тексту определ€ем пор€ковый номер ноды
	...
	// true when the file name starts with a numeric version prefix
	bool CheckScriptsVersion(const wchar_t* tested)
	{
		...
		if(separator <= 0 || separator == name.GetLength() - 1)
			return false;
	...
	{
		CString dst = L"";
		CString from = L"абвгдеЄжзийклмнопрстуфцчшщъыьэю€Ї≥њјЅ¬√ƒ≈®∆«»… ЋћЌќѕ–—“”‘÷„ЎўЏџ№Ёёя™≤ѓ";
