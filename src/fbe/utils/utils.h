...
namespace U // place all utilities into their own namespace
{
	struct ElTextHTML
	{
		...
		ElTextHTML(BSTR html, BSTR text) : html(html), text(text)
		{
		}
	};
  ...
  CString	GetModulePath(HMODULE module = NULL);
  ...
  T QueryBV(HKEY hKey, const TCHAR* name, T def)
  {
	  ...
	  if(::RegQueryValueEx(hKey, name, NULL, &type, NULL, &len) != ERROR_SUCCESS ||
		  type != REG_BINARY || len != sizeof(T))
		  return def;

	  T ret = {};
	  if(::RegQueryValueEx(hKey, name, NULL, &type,
		  reinterpret_cast<BYTE*>(&ret), &len) != ERROR_SUCCESS ||
		  type != REG_BINARY || len != sizeof(T))
		  return def;
