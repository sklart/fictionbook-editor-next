...
CString	GetProgDir() {
  ...
  DWORD len=::GetModuleFileName(g_hInstance,cp,MAX_PATH);
...
CString	GetCBString(HWND hWnd,int idx) {
  ...
  if (len==CB_ERR)
    return CString();
  CString ret;
  TCHAR	  *cp=ret.GetBuffer(static_cast<int>(len) + 1);
...
void InitSettings() {
  ...
  DWORD	  pathlen=::GetModuleFileName(g_hInstance,filepath,MAX_PATH);
  TCHAR	  *appname;
  if (pathlen==0)
