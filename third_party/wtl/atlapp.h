...
namespace RunTimeHelper
{
	...
	{
		HMODULE module = ::LoadLibrary(_T("comctl32.dll"));
		if(module == NULL)
			return false;

		DLLVERSIONINFO version = { sizeof(DLLVERSIONINFO) };
		DLLGETVERSIONPROC getVersion = reinterpret_cast<DLLGETVERSIONPROC>(
			::GetProcAddress(module, "DllGetVersion"));
		HRESULT result = getVersion != NULL ? getVersion(&version) : E_NOTIMPL;
		::FreeLibrary(module);
		return SUCCEEDED(result) && version.dwMajorVersion >= 6;
	}
	...
	{
		UINT nSize = sizeof(REBARBANDINFO);
